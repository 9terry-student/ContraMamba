# P3-W6-F2-P4-L Environment-Binding Retry1 Exact-Byte Provisioning Execution-Authority Candidate

Authority/version:

`P3W6F2P4L_CURRENT_EXACT_BYTE_PROVISIONING_ENVIRONMENT_BINDING_RETRY1_EXECUTION_AUTHORITY_CANDIDATE_V1`
...
training, evaluation, validation, commit, push, cleanup, reset, or GPU work. It
becomes executable only after independent verification and immutable freeze.

This candidate authorizes exactly one replacement CPU-only P4-L provisioning
attempt after the prior attempt
`p3w6f2-p4l-exact-byte-provision-67d9dcc` failed before staging or destination
mutation with
`P4L_PROVISION_BLOCKED:FREEZE_NOT_LOWERCASE_40_HEX`. The previous attempt is
consumed and must not be rerun. Failure of this retry1 attempt does not
authorize retry2, cleanup, deletion, reset, or any further mutation.

This recovery authority supersedes only the failed external/notebook
environment-injection mechanism for this retry1 attempt. It does not supersede
or weaken source identity, physical hashes, semantic hash, row count,
destination, exact children, raw-byte copy, same-parent staging,
`renameat2(RENAME_NOREPLACE)`, no-overwrite publication, staging retention on
failure, exact Git-status delta, tracked/index cleanliness, source byte
preservation, postflight checks, Linux-only behavior, CPU-only behavior, or the
no-builder/no-P4-Q/no-P4-W/no-training/no-evaluation/no-A0/no-GPU boundaries.

## 2. Authority chain consumed

This candidate consumes, in precedence order:

1. The current controller instruction for exact-byte provisioning
   environment-binding recovery retry1.
2. Repository HEAD `67d9dcc1f3524422a4b6c06de9e5596d08b961d2`.
3. `reports/reason_router_p2_p3w6f2_p4l_current_exact_byte_provisioning_freeze_binding_correction_execution_authority_spec_candidate.md`.
4. `reports/reason_router_p2_p3w6f2_p4l_current_exact_byte_provisioning_corrected_execution_authority_spec_candidate.md`.
5. `reports/PRE_URP_HANDOFF.md`, `docs/RESEARCH_OPERATIONS.md`, and
   `AGENTS.md`.

The prior attempt evidence consumed by this candidate is:

- run: `p3w6f2-p4l-exact-byte-provision-67d9dcc`
- command SHA256:
  `484019f41eb959d1484b936ec358696a11643c07f76ab3268bb12a83023ec78c`
- exit: `1`
- failure: `P4L_PROVISION_BLOCKED:FREEZE_NOT_LOWERCASE_40_HEX`
- files collected: `0`
- imported ZIP SHA256:
  `9e3c735f49914e32bce0c1c45df32396475c88ce3e3569e554e32de68ad0fa34`
- run log SHA256:
  `a5b4f336fc2e607ac5452d75eafec12bc97843ff5123f523e60a00c44682ee83`
- run meta SHA256:
  `024205809a31f9b4dbe7e8cd1317cc9c4df2b8f9a20c0ca5b517baa7d0461c8a`
- local import: `PASS`

## 3. Recovery delta

The retry1 command must not rely on notebook `%env` or any pre-existing
`P4L_PROVISION_AUTHORITY_FREEZE` value. The exact future command must derive
the freeze value command-locally from the checked-out repository HEAD by
beginning exactly:

```bash
P4L_PROVISION_AUTHORITY_FREEZE="$(git rev-parse HEAD)" CUDA_VISIBLE_DEVICES="" python - <<'PY'
```

Before any staging or destination mutation, the command must require the freeze
commit tree to contain all three authority paths:

1. `reports/reason_router_p2_p3w6f2_p4l_current_exact_byte_provisioning_freeze_binding_correction_execution_authority_spec_candidate.md`
2. `reports/reason_router_p2_p3w6f2_p4l_current_exact_byte_provisioning_corrected_execution_authority_spec_candidate.md`
3. `reports/reason_router_p2_p3w6f2_p4l_current_exact_byte_provisioning_environment_binding_retry1_execution_authority_spec_candidate.md`

All existing freeze checks are retained: lowercase 40-hex format, resolves to
a commit, object type is commit, and equals runtime HEAD.

## 4. Preserved source and destination contract
...
deleted by this authority. A failed provisioning attempt may leave only one
attempt-created `.p4l-staging-*` directory as failure evidence. Its existence
does not establish provisioning PASS, P4-L PASS, or any scientific result.

## 5. Complete exact future CPU-only retry1 command
...
AUTHORITY_RELATIVE = "reports/reason_router_p2_p3w6f2_p4l_current_exact_byte_provisioning_corrected_execution_authority_spec_candidate.md"
CORRECTION_AUTHORITY_RELATIVE = "reports/reason_router_p2_p3w6f2_p4l_current_exact_byte_provisioning_freeze_binding_correction_execution_authority_spec_candidate.md"
RECOVERY_AUTHORITY_RELATIVE = "reports/reason_router_p2_p3w6f2_p4l_current_exact_byte_provisioning_environment_binding_retry1_execution_authority_spec_candidate.md"
...
require(FREEZE is not None and re.fullmatch(r"[0-9a-f]{40}", FREEZE) is not None,
        "FREEZE_NOT_LOWERCASE_40_HEX")
require(git("cat-file", "-e", FREEZE + "^{commit}") == "", "FREEZE_COMMIT_UNRESOLVED")
require(git("cat-file", "-t", FREEZE).strip() == "commit", "FREEZE_NOT_COMMIT")
for authority_path, code in (
    (CORRECTION_AUTHORITY_RELATIVE, "FREEZE_MISSING_CORRECTION_AUTHORITY"),
    (AUTHORITY_RELATIVE, "FREEZE_MISSING_FINAL_AUTHORITY"),
    (RECOVERY_AUTHORITY_RELATIVE, "FREEZE_MISSING_RETRY1_RECOVERY_AUTHORITY"),
):
    tree_paths = git("ls-tree", "-r", "--name-only", FREEZE, "--", authority_path).splitlines()
    require(tree_paths == [authority_path], code)
require(git("rev-parse", "HEAD").strip() == FREEZE, "HEAD_MISMATCH")
...
require(git("rev-parse", "HEAD").strip() == FREEZE, "POST_HEAD_MISMATCH")
tracked_and_index_clean()
...
The command is complete; it contains no provisioning placeholder. The Python
body preserves the previously frozen provisioning implementation from the
final corrected executable authority, except for the authorized command-local
freeze derivation and the retry1 recovery-authority tree-membership binding.

## 6. Execution and interpretation boundary

A retry1 provisioning PASS would establish only exact-byte canonical
availability and source/destination identity. It would not establish formal A0
authority, A0 execution, training/evaluation success, scientific
effectiveness, promotion, or any A1/A2/A3 result.

After any future successful retry1 provisioning attempt, the next required
boundary remains a separately created, independently verified, and frozen
current-HEAD P4-L provisioning-result validation authority. This candidate does
not create or execute that validator.

## 7. Candidate stop conditions

This candidate is blocked if, at freeze or execution, any of the following is
true:

- HEAD differs from the exact freeze supplied command-locally by
  `git rev-parse HEAD`;
- tracked worktree or index is dirty;
- the prior consumed attempt is rerun;
- this retry1 recovery authority is not present in the freeze commit tree;
- either prior authority path is not present in the freeze commit tree;
...
- any mutation beyond the exact untracked canonical directory would be needed;
  or
- the command would require notebook `%env`, a pre-existing
  `P4L_PROVISION_AUTHORITY_FREEZE`, HEAD movement, builder/P4-Q/P4-W
  execution, parsing for copy, reconstruction, manual edit, cleanup, training,
  evaluation, A0, or GPU.

## 8. Final creation result

This candidate creation authorizes and runs nothing. It is CPU-only future
authority text; no provisioning, copying, Kaggle execution, training,
evaluation, A0, GPU, builder, P4-Q, P4-W rerun, commit, push, staging, cleanup,
or reset is authorized by creation. The candidate SHA256 and command SHA256 are
computed from this exact UTF-8 file after creation and reported by the creating
task.

Required readiness token:

`P4L_ENV_BINDING_RETRY1_AUTHORITY_READY`
