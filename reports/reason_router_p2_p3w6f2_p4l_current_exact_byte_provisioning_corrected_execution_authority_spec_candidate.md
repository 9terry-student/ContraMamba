# P3-W6-F2-P4-L Final Corrected Exact-Byte Provisioning Execution-Authority Candidate

Authority/version:

`P3W6F2P4L_CURRENT_EXACT_BYTE_PROVISIONING_CORRECTED_EXECUTION_AUTHORITY_CANDIDATE_V1`
...
the runtime value of `P4L_PROVISION_AUTHORITY_FREEZE`. The only permitted repository
...
2. Current repository HEAD
   the runtime value of `P4L_PROVISION_AUTHORITY_FREEZE`.
...
## 8. Exact command identity and execution contract

The exact command above is UTF-8 text with LF line endings. Its SHA256 is:

`484019f41eb959d1484b936ec358696a11643c07f76ab3268bb12a83023ec78c`

The required environment contract is exactly one assignment before execution:

`P4L_PROVISION_AUTHORITY_FREEZE=<lowercase 40-hex commit SHA>`

The value must resolve to a commit, contain both the frozen correction-authority path and this final executable-authority path, and equal runtime HEAD. The expected success token is:

`P3W6F2P4L_CURRENT_EXACT_BYTE_PROVISION_FREEZE_BOUND_PASS`

The only allowed Git-status delta after success is exactly the three untracked paths comprising the canonical destination directory and its two children. Tracked worktree and index status must remain clean. Any failure is fail-closed; no overwrite, rollback, cleanup, or retry disposition is authorized. Attempt-created staging is retained and reported on pre-publication failure; post-publication validation failure reports the canonical destination as provisioned-but-validation-failed.

## 9. Post-provision interpretation
...
## 10. Exact next validation boundary
...
## 13. Final creation result

This candidate creation authorizes and runs nothing. It is CPU-only future authority text; no training, evaluation, A0, GPU, builder, P4-Q, P4-W rerun, provisioning, copying, commit, push, staging, cleanup, or reset is authorized by creation. The candidate SHA256 is computed from this exact UTF-8 file after creation and is reported by the creating task.

Required readiness token:

`P4L_FINAL_CORRECTED_PROVISIONING_AUTHORITY_READY`
