...
From all 720 ordered rows the analyzer reconstructs clean CE, dense canonical prediction counts, clean accuracy, macro-F1, SUPPORT recall, false entitlement, false not-entitled, and polarity error. The authoritative clean CE reduction exactly reproduces the trainer contract: `torch.tensor(values, dtype=torch.float32, device="cpu").mean().item()`, where `values` are the 720 ordered exported `final_ce` values. The resulting Python scalar must equal trajectory `clean_dev_ce` exactly. Python summation, `math.fsum`, NumPy float64 means, and torch float64 means are not authoritative CE gates, and the CE tolerance is not widened. PyTorch absence or float32 reduction failure remains fail-closed.

For every epoch, `reconstructed_clean_dev_ce_matches` records `epoch`, `final_ce_row_count=720`, `authoritative_reduction_dtype="torch.float32"`, `authoritative_reduction_device="cpu"`, `reconstructed_torch_float32_mean`, `trajectory_clean_dev_ce`, `exact_float32_match`, `diagnostic_python_float64_mean`, and `diagnostic_python_float64_abs_diff`. The Python/float64 values are diagnostic only. At report level, `clean_dev_ce_reduction_contract` records the ordered export row source, row count 720, torch.float32 dtype, CPU device, mean reduction, and exact comparison.

The independent `epoch_export_non_ce_metrics_and_gold_topology` gate covers the strict epoch-export schema, prediction counts, accuracy, macro-F1, SUPPORT recall, false entitlement, false not-entitled, polarity error, gold total 720, and gold SUPPORT total 89. Accuracy, macro-F1, and SUPPORT recall retain the fixed `1e-7` absolute/relative tolerance. Reconstructed count/error fields are exact. Trajectory false-entitlement, false-not-entitled, polarity-error, and all three prediction counts remain exact non-bool integers. A CE mismatch does not produce a duplicate non-CE metric or topology failure.

This reduction correction was precommitted after the first Kaggle execution showed exact torch.float32 agreement in all 120 run/epoch cells, while Python/float64 summation differed by at most `1.3187527658775622e-07` and exceeded the prior tolerance only for `seed176_baseline` epoch 5. It corrects the diagnostic reduction dtype only and does not alter any replay artifact, checkpoint, model result, or decision criterion.
