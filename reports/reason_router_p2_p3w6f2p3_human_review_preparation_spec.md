...
No `AUTO_ACCEPT`, blanket confirmation, batch confirmation, cluster-level approval, or approval by structural similarity is allowed under `P3W5_F2_MANUAL_REVIEW_V1`. Action on one pair never approves another pair. If the human independently chooses the same values for an escalated case, `CONFIRM` remains allowed only after pair-level inspection.

Narrow V2 supersession: `P3W6F2_HYBRID_HUMAN_REVIEW_V2`, specified in `reports/reason_router_p2_p3w6f2p3_structural_cohort_confirmation_spec.md`, permits `STRUCTURAL_COHORT_CONFIRMATION` only for the exact frozen F2 cohort, only after the exact 20 individually reviewed audit pairs validate as `V/V/V/M` with seven-predicate structural coverage and zero overrides, only after the deterministic `P3W6F2_STRUCTURAL_COHORT_GATE_V1` passes, only after the AI prescreen artifact validates as `V/V/V/M` and `CLEAR_SUGGESTION`, and only after an explicit `COHORT_CONFIRM` action. This supersession does not claim the remaining pairs were individually inspected and does not weaken the individual-review boundary for any other cohort, protocol, or future paper analysis.

The V2 structural path is two phase: `cohort-audit` first writes a stable external audit artifact with a canonical SHA-256 over source authority, source-record hashes, AI-prescreen artifact identity, exact validated 20-record WIP state, eligible pairs, exception pairs, and structural gate results. Only after a human has reviewed that artifact may `cohort-confirm` run with `--audit-path`, `--expected-audit-sha256`, and `--ack-structural-cohort-confirm`; confirmation recomputes the audit and fails closed on any drift. Structural WIP records must match the loaded cohort artifact's exact `cohort_confirmed_pair_ids`, never merely a matching cohort ID, and the confirmation artifact plus WIP ledger are written through rollback-safe staging.

V2 timestamp provenance is truthful: CLI individual records use `CAPTURED_IN_RECORD`; imported XLSX-confirmed individual records use `NOT_CAPTURED_IN_XLSX`; structural cohort records use `NOT_APPLICABLE_STRUCTURAL_COHORT`. In all V2 cases, `reviewed_at_utc` remains a compatibility alias of `authority_recorded_at_utc` and is not fabricated historical visual-inspection time.
