...
## B2-B2 static and epoch schema separation

`stage196b2b2_row_path_summary.csv` is the authoritative 16-row, 61-column static table. `stage196b2b2_epoch_paired_paths.csv` is the authoritative 320-row, 53-column epoch-varying table. Their exact native schemas are declared separately from the frozen B2-B2 headers. `path_class`, tail-three statuses, terminal deltas, and row-level path metadata are required only from the static summary; epoch, joint/intervention channel trajectories, deltas, pass transitions, predictions, and SUPPORT-vs-NOT_ENTITLED margins are required natively from the paired table. B2-B3-derived enrichment is never represented as a historical source column.

The shared identity key is exactly `seed`, `stable_row_id`, `id`, `source_row_id`, `dev_position`, `transition_role`, and `intervention_type`. Seed and dev position receive only integer representation normalization; identifiers are never fuzzy-matched. The row summary must contain exactly one non-null supported-path row for each of 16 frozen identities. The paired table must contain exactly one row for every identity and epoch 1 through 20, with no orphan or missing identities.

B2-B3 builds an in-memory many-to-one index from the unique row summary and joins it to the paired rows by the full key. Before collapsing duplicated columns, it compares both source values for all seven identity fields and the four recurrent-membership fields present in both native schemas. Any conflict fails closed with paired and row-summary values retained in contract evidence. The enriched table must retain 320 rows and receive `path_class` directly from its unique row-summary donor; no predecessor file is changed and `path_class` is never recomputed from epoch values or treated as a composer input.

The contract records `b2b2_row_summary_schema_closure`, `b2b2_row_summary_identity_uniqueness`, `b2b2_row_summary_path_class_closure`, `b2b2_paired_epoch_schema_closure`, `b2b2_paired_epoch_identity_epoch_uniqueness`, `b2b2_paired_epoch_identity_closure`, `b2b2_exact_twenty_epochs_per_identity`, `b2b2_static_metadata_consistency`, `b2b2_row_to_epoch_many_to_one_join`, and `b2b2_enriched_paired_path_class_closure`. Evidence retains row and identity counts, the full key, epoch coverage, duplicate/orphan/missing counts, static conflicts, enriched row count, null path-class count, and pass state. Column, identity, identity-epoch, orphan, missing, and static-conflict failures have separate diagnostics.
...
Before resolution, the call site maps the loader's exact producer keys into one structural manifest and records `historical_provenance_context_closure` with required, available, and missing object names only. A missing loaded object fails precisely as an internal context error. After this gate passes, every analysis, contract, path, normalized-role mapping, presence flag, and explicit B2-B2 commit is supplied to the resolver as a named keyword argument. The resolver never indexes a predecessor or context dictionary and never reloads an artifact.
