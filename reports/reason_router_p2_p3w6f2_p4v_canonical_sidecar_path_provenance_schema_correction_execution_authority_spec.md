# P3-W6-F2-P4-V Canonical-Sidecar-Path Provenance Schema Correction Execution Authority Candidate

Authority/version:

`P3W6F2P4V_CANONICAL_SIDECAR_PATH_PROVENANCE_SCHEMA_CORRECTION_EXECUTION_AUTHORITY_CANDIDATE_V1`
...
Only after independent static verification PASS and immutable P4-V freeze may
...
- P4-U execution authority freeze:
  `8a841d3989c65c9cc575ea58b06734cf69d262b3`.
- Observed P4-U failure:
  `P4U_FAIL:PROVENANCE_CANONICAL_SIDECAR_PATH_MISMATCH`.
- Frozen P4-L:
...
`P4-V EXECUTION AUTHORITY CANDIDATE CREATION ONLY`
...
P4-U remains FAIL with exact observed token:

`P4U_FAIL:PROVENANCE_CANONICAL_SIDECAR_PATH_MISMATCH`

P4-V does not retroactively relabel P4-T PASS and does not assert that the
canonical P4-L artifact is valid until the future authorized validator actually
prints the P4-V PASS token after freeze.
...
The P4-U failure is preserved as a validator-schema failure: P4-U inherited
`canonical_sidecar_path` from P4-S, but the exact frozen builder provenance
dictionary at `2f9e6076791358922e3ebd70e89533d9cb83b458` emits
`sidecar_path` and does not emit `canonical_sidecar_path`. This does not
establish an artifact defect.

## 3. Scope

P4-V starts from the P4-T validator semantics and preserves every valid P4-T
check, including:
...
- authority identity, environment variable, authority path, and PASS token to
  the P4-V namespace;
- the invalid sidecar provenance assertion from `canonical_sidecar_path` to
  the frozen builder field `sidecar_path`, with required value
  `OUTPUT_DIR_REL + "/" + SIDECAR_NAME`.

P4-V must not require the alias provenance keys `source_physical_sha256` or
`source_semantic_sha256`; absence of either alias must not be treated as a
failure.

P4-V must not require `canonical_sidecar_path`; absence of that non-builder
alias must not be treated as a failure.

## 4. Runtime Authority Boundary

After independent verification PASS and immutable P4-V freeze, the workflow
controller must supply exactly one runtime parameter:

`P4V_AUTHORITY_FREEZE`

The future validator must require:

- `P4V_AUTHORITY_FREEZE` is set.
- `P4V_AUTHORITY_FREEZE` is exactly 40 lowercase hex characters.
- `git cat-file -e "${P4V_AUTHORITY_FREEZE}^{commit}"` succeeds.
- this P4-V execution-authority path exists at that commit.
- the validator does not check out, fetch, reset, clean, or otherwise
  materialize the P4-V freeze.
- `git rev-parse HEAD ==
  2f9e6076791358922e3ebd70e89533d9cb83b458`.
...
AUTHORITY_VERSION = "P3W6F2P4V_CANONICAL_SIDECAR_PATH_PROVENANCE_SCHEMA_CORRECTION_EXECUTION_AUTHORITY_V1"
PASS_TOKEN = "P3W6F2P4V_CANONICAL_SIDECAR_PATH_PROVENANCE_SCHEMA_CORRECTION_PASS"

EXECUTION_HEAD = "2f9e6076791358922e3ebd70e89533d9cb83b458"
P4U_EXECUTION_AUTHORITY_FREEZE = "8a841d3989c65c9cc575ea58b06734cf69d262b3"
...
P4V_AUTHORITY_PATH = "reports/reason_router_p2_p3w6f2_p4v_canonical_sidecar_path_provenance_schema_correction_execution_authority_spec.md"
...
def fail(code: str) -> None:
    raise SystemExit(f"P4V_FAIL:{code}")
...
authority_freeze = os.environ.get("P4V_AUTHORITY_FREEZE", "")
require(re.fullmatch(r"[0-9a-f]{40}", authority_freeze or "") is not None, "P4V_AUTHORITY_FREEZE_INVALID")
run_git(["cat-file", "-e", f"{authority_freeze}^{{commit}}"])
authority_listing = run_git(["ls-tree", "-r", "--name-only", authority_freeze, "--", P4V_AUTHORITY_PATH])
require(authority_listing == P4V_AUTHORITY_PATH, "P4V_AUTHORITY_PATH_MISSING_AT_FREEZE")
...
required_provenance = {
    ...
    "sidecar_path": OUTPUT_DIR_REL + "/" + SIDECAR_NAME,
}
...
## 6. Provenance Schema Audit

Static audit authority:

- exact frozen builder/execution HEAD:
  `2f9e6076791358922e3ebd70e89533d9cb83b458`;
- exact builder file:
  `scripts/build_reason_router_p3w6f2_p4l_current_lineage_integrity_sidecar.py`;
- exact builder provenance dictionary:
  `build_provenance(...) -> dict[str, Any]`, lines containing the returned
  dictionary at frozen HEAD.
...
No executable assertion requires `canonical_sidecar_path`. The frozen builder
dictionary emits `sidecar_path`:

```python
"sidecar_path": f"{output_dir.as_posix()}/{SIDECAR_NAME}",
```

Key-by-key audit of every provenance key asserted by the future validator:

| Future validator asserted provenance key | Expected value source in validator | Frozen builder-schema proof | Audit |
| --- | --- | --- | --- |
| `sidecar_physical_sha256` | `EXPECTED_SIDECAR_PHYSICAL` | returned as `"sidecar_physical_sha256": sidecar_physical_sha256` | PASS |
| `sidecar_semantic_sha256` | `EXPECTED_SIDECAR_SEMANTIC` | returned as `"sidecar_semantic_sha256": sidecar_semantic_sha256` | PASS |
| `schema_version` | `P3W6F2P4L_CURRENT_LINEAGE_INTEGRITY_SIDECAR_PROVENANCE_V1` | returned as `"schema_version": PROVENANCE_SCHEMA_VERSION` | PASS |
| `authority_version` | `P3W6F2P4L_CURRENT_LINEAGE_INTEGRITY_ARTIFACT_CONTRACT_AUTHORITY_V1` | returned as `"authority_version": AUTHORITY_VERSION` | PASS |
| `p4l_authority_commit` | `P4L_AUTHORITY_COMMIT` / `80cb034792f03226cf6e22c196c1229ed4e6dd62` | returned as `"p4l_authority_commit": P4L_AUTHORITY_COMMIT` | PASS |
| `builder_source_commit` | `EXECUTION_HEAD` / `2f9e6076791358922e3ebd70e89533d9cb83b458` | returned as `"builder_source_commit": builder_commit` | PASS |
| `builder_source_sha256` | `BUILDER_SOURCE_SHA256` | returned as `"builder_source_sha256": builder_source_sha256` | PASS |
| `source_dataset_path` | `SOURCE_REL` | returned as `"source_dataset_path": SOURCE_DATASET_PATH` | PASS |
| `source_dataset_sha256` | `EXPECTED_SOURCE_PHYSICAL` | returned as `"source_dataset_sha256": SOURCE_DATASET_SHA256` | PASS |
| `source_dataset_semantic_sha256` | `EXPECTED_SOURCE_SEMANTIC` | returned as `"source_dataset_semantic_sha256": SOURCE_DATASET_SEMANTIC_SHA256` | PASS |
| `p4b_compatibility_rows_path` | `P4B_ROWS_REL` | returned as `"p4b_compatibility_rows_path": P4B_COMPATIBILITY_ROWS_PATH` | PASS |
| `p4b_compatibility_rows_sha256` | `EXPECTED_P4B_ROWS_PHYSICAL` | returned as `"p4b_compatibility_rows_sha256": P4B_COMPATIBILITY_ROWS_SHA256` | PASS |
| `p4b_compatibility_summary_path` | `P4B_SUMMARY_REL` | returned as `"p4b_compatibility_summary_path": P4B_COMPATIBILITY_SUMMARY_PATH` | PASS |
| `p4b_compatibility_summary_sha256` | `EXPECTED_P4B_SUMMARY_PHYSICAL` | returned as `"p4b_compatibility_summary_sha256": P4B_COMPATIBILITY_SUMMARY_SHA256` | PASS |
| `p4b_compatibility_provenance_path` | `P4B_PROVENANCE_REL` | returned as `"p4b_compatibility_provenance_path": P4B_COMPATIBILITY_PROVENANCE_PATH` | PASS |
| `p4b_compatibility_provenance_sha256` | `EXPECTED_P4B_PROVENANCE_PHYSICAL` | returned as `"p4b_compatibility_provenance_sha256": P4B_COMPATIBILITY_PROVENANCE_SHA256` | PASS |
| `sidecar_path` | `OUTPUT_DIR_REL + "/" + SIDECAR_NAME` | returned as `"sidecar_path": f"{output_dir.as_posix()}/{SIDECAR_NAME}"` | PASS |
| `p4b_compatibility_authorized_scope` | object with `pair_count == 119`, `member_count == 357` | returned as `"p4b_compatibility_authorized_scope": {"pair_count": P4B_AUTHORIZED_PAIR_COUNT, "member_count": P4B_AUTHORIZED_MEMBER_COUNT}` | PASS |
| `provenance_physical_sha256_self_certified` | `False` | returned as `"provenance_physical_sha256_self_certified": False` | PASS |
| `row_count` | `3600` | returned as `"row_count": EXPECTED_ROW_COUNT` | PASS |
| `one_to_one_row_coverage` | `True` | returned as `"one_to_one_row_coverage": True` | PASS |
| `unique_row_id` | `True` | returned as `"unique_row_id": True` | PASS |
| `historical_stage185_used_as_current_source_identity` | `False` | returned as `"historical_stage185_used_as_current_source_identity": False` | PASS |
| `blockers` | `[]` | returned as `"blockers": list(blockers)`, with default `blockers: Sequence[str] = ()` | PASS |
| `failure_reasons` | `[]` | returned as `"failure_reasons": list(failure_reasons)`, with default `failure_reasons: Sequence[str] = ()` | PASS |
| `artifact_materialization_authorized_by_p4l` | `False` | returned as `"artifact_materialization_authorized_by_p4l": False` | PASS |
| `kaggle_authorized` | `False` | returned as `"kaggle_authorized": False` | PASS |
| `gpu_authorized` | `False` | returned as `"gpu_authorized": False` | PASS |
| `training_admission_released` | `False` | returned as `"training_admission_released": False` | PASS |
| `a0_execution_authorized` | `False` | returned as `"a0_execution_authorized": False` | PASS |
| `training_authorized` | `False` | returned as `"training_authorized": False` | PASS |
| `evaluation_authorized` | `False` | returned as `"evaluation_authorized": False` | PASS |

Non-asserted invalid inherited key:

| Key | Disposition | Frozen builder-schema proof | Audit |
| --- | --- | --- | --- |
| `canonical_sidecar_path` | not asserted by P4-V validator | absent from the exact frozen builder `build_provenance` return dictionary; replaced by `sidecar_path` | PASS |

`PROVENANCE_SCHEMA_AUDIT = PASS`
...
`P3W6F2P4V_CANONICAL_SIDECAR_PATH_PROVENANCE_SCHEMA_CORRECTION_PASS`
...
- index clean;
- exactly one new untracked P4-V execution-authority spec;
- frozen P4-U execution authority unchanged;
- no Python, pytest, builder, materialization, validation, training,
  evaluation, Kaggle, commit, or push.

Candidate path:

`reports/reason_router_p2_p3w6f2_p4v_canonical_sidecar_path_provenance_schema_correction_execution_authority_spec.md`
...
`P4V_EXECUTION_AUTHORITY_CANDIDATE_READY_FOR_VERIFICATION`
