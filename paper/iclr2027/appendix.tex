...
This appendix transcribes the frozen experimental records. Figure source
classes are mapped in Appendix~\ref{app:provenance}. No new model execution
or statistical tests are involved.
...
XG2/XG4 response-free directions define independent principal planes
within each model's native strong-channel mask. For the plotted kernel
quantity, let $k_j=\texttt{conv\_weight}[j,0,\mathrm{lag0}]$,
$\mu_{k^2}=\operatorname{mean}_j k_j^2$, and
$\mathrm{strong}=\{j:k_j^2>\mu_{k^2}\}$.
Geometry preparation uses Mamba-side forward activations without XG1
response access, downstream task logits or gradients, or backward passes.
The inputs belong to the research families, so this does not characterize
all natural language. The historical 130M strong-channel rule is the same:
$\mu_{k^2}=0.027899337798707836$ and 395 of 1536 channels are strong.
Its response-blind projector-contrast reconstruction gives leading positive
eigenvalue $\lambda_1=0.8706181418918275$. These two frozen quantities
are plotted with open markers in Figure~\ref{fig:scale}. The 130M
reconstruction predates the homogeneous 370M--2.8B geometry bridge;
the five plotted points are not one matched execution. No derived 130M
spectral-share or entropy summary is added.
...
where each $E$ is the mean squared susceptibility over its five frozen
directions under condition $c$. XG1 outcomes do not refit the basis.
...
Figure~\ref{fig:deep} draws the historical 130M causal synthesis,
specificity inference, seed-181 behavioral bridge, and frozen pair-level
readout--behavior merge. Figure~\ref{fig:scale} draws the historical
130M/370M conclusions, later 790M/1.4B/2.8B confirmation inference,
the historical 130M kernel mean-square and leading-plane eigenvalue,
and the later response-blind four-scale native-geometry bridge.
Figure~\ref{fig:objective} draws the five-scale task-objective readout
synthesis, original four-scale vanilla-LM analysis, and later 130M
completeness analysis. Figure~\ref{fig:overview} summarizes these
same frozen evidence classes. The 130M geometry point has earlier
response-blind provenance than the later matched bridge; the 130M
vanilla-LM result is a post-primary completeness extension.
