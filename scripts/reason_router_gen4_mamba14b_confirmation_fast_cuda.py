...
CONFIRMATION_HOLDOUT_FREEZE_COMMIT = "e988cf3e20990a9397b8238df0a1edaf03674522"
...
CONFIRMATION_ROOT = Path(
    "data/reason_router_gen4_mamba14b_xg1_confirmation_v1"
)
CONFIRMATION_SOURCE_SHA256 = (
    "92c8c4c4c4ed61f33465b609a1466992214b9340822422659b04dafec2ddd6ea"
)
CONFIRMATION_ROWS_SHA256 = (
    "d9641443f395fe622d37634a3e58e7fcd4e512897264c265249908104c8069ed"
)
CONFIRMATION_MANIFEST_SHA256 = (
    "dd6f35639e69f6e87fd85eb8f7debb00fc97f803309ef71c178f790ade4ea2a4"
)
...
SHARDS = (
    {
        ...
        "start_index": 0,
        "end_index": 150,
        "pair_first": "xg1_fact_4201",
        "pair_last": "xg1_fact_4350",
        "pair_count": 150,
    ...
    },
    {
        ...
        "start_index": 150,
        "end_index": 300,
        "pair_first": "xg1_fact_4351",
        "pair_last": "xg1_fact_4500",
        "pair_count": 150,
        "forward_budget": FORWARDS_PER_SHARD,
    },
)
...
ALPHA = 0.05
SUCCESS_LABEL = "MAMBA14B_CORE_CONFIRMATION_SUPPORTED"
FAILURE_LABEL = "MAMBA14B_CORE_CONFIRMATION_NOT_SUPPORTED"
...
RESULT_PASS = "PASS_MAMBA14B_CORE_CONFIRMATION"
ITEM_SCHEMA = "gen4-mamba14b-core-confirmation-item-v1"
INFERENCE_SCHEMA = "gen4-mamba14b-core-confirmation-inference-v1"
MANIFEST_SCHEMA = "gen4-mamba14b-core-confirmation-manifest-v1"
...
def validate_protocol() -> None:
    ...
    require(CONTROL_PLANE == "P4", "CONTROL_PLANE")
...
def load_frozen_selection() -> dict[str, Any]:
    ...
    require(selection["result"] == "PASS_MAMBA14B_DISCOVERY_SELECTION_FREEZE", "DISCOVERY_SELECTION_RESULT")
...
def load_confirmation_population() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    holdout_builder.m370.prior.base.validate_materialized_rows(materialized, expected_pairs=PAIR_COUNT)
    require(manifest["schema_version"] == "GEN4_MAMBA14B_XG1_PROSPECTIVE_HOLDOUT_V1", "CONFIRMATION_SCHEMA")
    require(manifest["result"] == "PASS_MAMBA14B_XG1_CONFIRMATION_4201_4500_STRUCTURAL", "CONFIRMATION_RESULT")
    ...
    require(manifest["residual_response_access_allowed"] is False, "RESIDUAL_RESPONSE_BOUNDARY")
...
def run_pair(
    *, pair_index: int, pair: str, bases: Mapping[str, torch.Tensor], **kwargs: Any,
    ...
    return {
        ...
        "residual_response_accessed": False,
...
def infer_confirmation(items: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "result": RESULT_PASS,
        "primary_test": {
            "endpoint": "D_CORE=Q_restored(P5)-Q_control(P4)",
            "null": "E[D_CORE] <= 0",
...
def worker_run(
    *,
    ...
    try:
        ...
        meta = {
            "schema_version": "gen4-mamba14b-core-confirmation-worker-v1",
            "shard_id": shard_id,
...
def run_confirmation(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="gen4_mamba14b_core_confirmation_"
    ) as tmp:
        temp_dir = Path(tmp)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=("Mamba-1.4B prospective core confirmation on XG1 4201..4500. Uses frozen discovery P5/P4 selection and executes exactly one one-sided one-sample Student t-test on D_CORE."))
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print("CONFIRMATION_RANGE=xg1_fact_4201..xg1_fact_4500")
    print("SELECTION_REOPENED=False")
    print("RESIDUAL_RESPONSE_ACCESSED=False")
