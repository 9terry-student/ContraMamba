...
HF_REVISION = "96c48e0292b63f5346b6d30061af2551f7101e26"
...
COMPACT_CHECKPOINT_REL = Path(
    "reports/reason_router_gen4_mamba28b_training_runs/"
    "g4k-mamba28b-train-g3d-seed181-dualt4-hostcache-aaafcf1-retry2/"
...
TRAINING_ARTIFACT_FREEZE_COMMIT = (
...
RESULT_PASS = "PASS_MAMBA28B_GEOMETRY_PREPARATION"
...
def validate_protocol_constants() -> None:
    require(INTERVENTION_LAYER == 47, "INTERVENTION_LAYER_MAPPING")
    require(
...
def reconstruct_model(
    *,
    ...
    source_selected = compact.get("source_selected_checkpoint")
    ...
    require(isinstance(source_selected, Mapping), "SOURCE_SELECTED_META")
    ...
    require(
        int(pretrained.get("state_key_count", -1)) == 642,
        "PRETRAINED_STATE_KEY_COUNT",
    )
    require(
        pretrained.get("omitted_from_selected_checkpoint") is True,
        "PRETRAINED_OMISSION_FLAG",
    )

    require(
        source_selected.get("sha256")
        == SOURCE_SELECTED_CHECKPOINT_SHA256,
        "SOURCE_SELECTED_CHECKPOINT_SHA256",
    )
    require(
        source_selected.get("selected_state_canonical_sha256")
        == DOWNSTREAM_STATE_CANONICAL_SHA256,
        "SOURCE_SELECTED_STATE_CANONICAL_SHA256",
    )
    require(
        int(source_selected.get("state_key_count", -1)) == 36,
        "SOURCE_SELECTED_STATE_KEY_COUNT",
    )
    require(
        source_selected.get("contains_pretrained_backbone") is False,
        "SOURCE_SELECTED_CONTAINS_BACKBONE",
    )
    require(
        source_selected.get("representation")
        == "downstream_only_selected_state_with_exact_pinned_backbone_omitted",
        "SOURCE_SELECTED_REPRESENTATION",
    )
    ...
    require(
        ...
        and reconstruction.get("scientific_forward_executed") is False
        and reconstruction.get(
            "training_executed_during_compaction"
        ) is False,
    ...
    require(
        not any(key.startswith("mamba.") for key in downstream),
        "DOWNSTREAM_BACKBONE_LEAK",
    )
    ...
    require(
        len(model.mamba.state_dict()) == 642,
        "MAMBA_STATE_KEY_COUNT",
    )

    current_state = model.state_dict()
    require(len(current_state) == 678, "FULL_STATE_KEY_COUNT_BEFORE")
    ...
    require(len(backbone_keys) == 642, "BACKBONE_KEY_COUNT")
    require(len(downstream_keys) == 36, "MODEL_DOWNSTREAM_KEY_COUNT")
    require(set(downstream) == downstream_keys, "DOWNSTREAM_KEY_SET")
    ...
    require(
        set(loaded.missing_keys) == backbone_keys,
        "DOWNSTREAM_LOAD_MISSING_KEYS",
    )
    require(
        not loaded.unexpected_keys,
        "DOWNSTREAM_LOAD_UNEXPECTED_KEYS",
    )
    ...
    require(
        canonical_state_sha256(loaded_downstream)
        == DOWNSTREAM_STATE_CANONICAL_SHA256,
        "LOADED_DOWNSTREAM_CANONICAL_SHA256",
    )
    ...
    return model, kernels, {
        ...
        "downstream_state_canonical_sha256":
            DOWNSTREAM_STATE_CANONICAL_SHA256,
        "backbone_state_key_count": len(backbone_keys),
        "downstream_state_key_count": len(downstream_keys),
        "full_state_key_count": len(model.state_dict()),
        "backbone_loaded_from_exact_pinned_snapshot": True,
        "downstream_loaded_exactly": True,
    }
...
def run_family(
    *,
    ...
    for pair in pairs:
        ...
        items.append({
            "schema_version": "gen4-mamba28b-geometry-family-item-v1",
            "family_key": family,
            "source_pair_id": pair,
...
def worker_run(
    *,
    ...
    try:
        ...
        meta = {
            "schema_version": "gen4-mamba28b-geometry-worker-v1",
            "gpu_id": gpu_id,
            "family_key": family,
...
def write_output_bundle(
    *,
    ...
    for payload in worker_payloads:
        require(
            payload["model_forward_count"] == FORWARDS_PER_FAMILY,
            "WORKER_BUDGET",
        )
        ...
        require(
            provenance["compact_checkpoint_sha256"]
            == COMPACT_CHECKPOINT_SHA256,
            "COMPACT_CHECKPOINT_SHARD_MISMATCH",
        )
        require(
            provenance["source_selected_checkpoint_sha256"]
            == SOURCE_SELECTED_CHECKPOINT_SHA256,
            "SOURCE_SELECTED_SHARD_MISMATCH",
        )
        require(
            provenance["pretrained_backbone_state_stream_sha256"]
            == PRETRAINED_BACKBONE_STATE_STREAM_SHA256,
            "BACKBONE_STREAM_SHARD_MISMATCH",
        )
        require(
            provenance["downstream_state_canonical_sha256"]
            == DOWNSTREAM_STATE_CANONICAL_SHA256,
            "DOWNSTREAM_STATE_SHARD_MISMATCH",
        )
        require(
            provenance["backbone_state_key_count"] == 642
            and provenance["downstream_state_key_count"] == 36
            and provenance["full_state_key_count"] == 678,
            "MODEL_STATE_KEY_COUNTS",
        )
        require(
            provenance["backbone_loaded_from_exact_pinned_snapshot"] is True
            and provenance["downstream_loaded_exactly"] is True,
            "MODEL_RECONSTRUCTION_FLAGS",
        )

        model_provenances.append(provenance)
    ...
    require(
        model_provenances[0] == model_provenances[1],
        "MODEL_PROVENANCE_SHARD_MISMATCH",
    )
    ...
    strong_indices = {
        "schema_version": "gen4-mamba28b-strong-mask-v1",
    ...
    summary = {
        "schema_version": "gen4-mamba28b-geometry-preparation-summary-v1",
        "result": RESULT_PASS,
        ...
        "model": {
            ...
            "compact_checkpoint_sha256": COMPACT_CHECKPOINT_SHA256,
            "source_selected_checkpoint_sha256": SOURCE_SELECTED_CHECKPOINT_SHA256,
            ...
            "training_artifact_freeze_commit": TRAINING_ARTIFACT_FREEZE_COMMIT,
    ...
    manifest = {
        "schema_version": "gen4-mamba28b-geometry-preparation-manifest-v1",
        "result": RESULT_PASS,
        "execution_head": expected_head,
...
def run_geometry_preparation(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="gen4_mamba28b_geometry_"
    ) as tmp:
        temp_dir = Path(tmp)
        ...
        for gpu_id, family in enumerate(FAMILIES):
            process = ctx.Process(
                ...
                name=f"mamba28b-geometry-{family}-gpu{gpu_id}",
            )
            process.start()
...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Independent Mamba-2.8B XG2/XG4 geometry reconstruction. "
