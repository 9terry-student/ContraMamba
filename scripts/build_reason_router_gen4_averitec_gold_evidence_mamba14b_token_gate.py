...
REQUIRED_PLAN_COMMIT = "f174b092a06b1d0973fa90c90856b2aa23b3526d"
...
SCALE = "mamba14b"
PRIMARY_P_VALUE_COUNT_PLANNED = 1
PRIMARY_ALTERNATIVE = "less"
PRIMARY_MULTIPLICITY = "none"
FAMILY_ALPHA = 0.05
HISTORICAL_AVERITEC_FAMILY_REOPENED = False

FROZEN_CAUSAL_OBJECT = {
    "scale": SCALE,
...
COHORT_SCHEMA = "GEN4_AVERITEC_GOLD_EVIDENCE_MAMBA14B_COMPATIBLE_COHORT_V1"
MANIFEST_SCHEMA = "GEN4_AVERITEC_GOLD_EVIDENCE_MAMBA14B_BOUNDARY_TOKEN_GATE_V1"
PASS_RESULT = "PASS_462_OF_462"
BLOCK_RESULT = "BLOCKED_MAMBA14B_EXTERNAL_TOKEN_GATE"
...
class AVeriTeC14BGateError(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise AVeriTeC14BGateError(message)
...
def authenticate_repo(expected_head: str) -> None:
    ...
    rc = subprocess.call(
        ["git", "merge-base", "--is-ancestor", REQUIRED_PLAN_COMMIT, expected_head],
        cwd=ROOT,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
    require(rc == 0, "PLAN_COMMIT_NOT_ANCESTOR")
    require(PLAN_PATH.is_file(), "PLAN_MISSING")
    require(
        hashlib.sha256(PLAN_PATH.read_bytes()).hexdigest() == PLAN_SHA256,
        "PLAN_SHA256",
    )
...
def download_pinned_source(*, url: str = UPSTREAM_RAW_URL) -> bytes:
...
def _token_ids(tokenizer: Any, text: str) -> list[int]:
    ...
    return [int(value) for value in encoded.ids]
...
def gate_row(
    *,
    ...
    serialized_length = len(consumed_claim) + 1 + len(consumed_evidence)
...
def provision_tokenizer_snapshot() -> Path:
    try:
        ...
        raise AVeriTeC14BGateError("HUGGINGFACE_HUB_REQUIRED") from exc
...
def load_tokenizer(snapshot: Path) -> tuple[Any, dict[str, Any]]:
    try:
        ...
        raise AVeriTeC14BGateError("TOKENIZERS_PACKAGE_REQUIRED") from exc
...
def build_manifest(
    *,
    ...
    return {
        ...
        "result": result,
        "phase": "MAMBA14B_AVERITEC_NEGATIVE_SIGN_RESPONSE_BLIND_TOKEN_GATE",
        "prospective_plan": {
        ...
        "primary_external_transfer_extension": {
            "scale": SCALE,
            "primary_endpoint": "D_EXT_14B=M_native-M_control",
            "alternative": PRIMARY_ALTERNATIVE,
            "family_alpha": FAMILY_ALPHA,
            "primary_p_value_count_planned": PRIMARY_P_VALUE_COUNT_PLANNED,
            "multiplicity": PRIMARY_MULTIPLICITY,
            "historical_130m370m_averitec_family_reopened":
                HISTORICAL_AVERITEC_FAMILY_REOPENED,
            "historical_p_values_in_new_family": 0,
        },
        "frozen_causal_object": FROZEN_CAUSAL_OBJECT,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Provision the pinned AVeriTeC development source, derive the frozen "
            "three-label gold-evidence cohort, and execute the response-blind "
            "Mamba-1.4B claim/evidence-boundary tokenizer gate. "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional already-provisioned exact Mamba-1.4B tokenizer snapshot. "
            "If omitted, only tokenizer files are downloaded from the frozen revision."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("SELECTED_PLANE=P5")
    print("CONTROL_PLANE=P4")
    print("INTERVENTION_LAYER=35")
    print("PRIMARY_ENDPOINT=D_EXT_14B=M_native-M_control")
    print("PRIMARY_ALTERNATIVE=less")
    print("PRIMARY_P_VALUE_COUNT_PLANNED=1")
    print("PRIMARY_MULTIPLICITY=none")
