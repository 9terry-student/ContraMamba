...
def load_source(root:Path,path:Path)->list[dict[str,Any]]:
 try:raw=subprocess.check_output(["git","show",f"{AUTHORITY_COMMIT}:{SOURCE_REL}"],cwd=root)
...
def main(argv:list[str]|None=None)->int:
 ...
 runtime=git_provenance(root);source=require_frozen_source_path(root,Path(a.source_data));snapshot,hf=resolve_hf_snapshot(a.hf_revision);pool,checked=prepare_candidate_pool(load_source(root,source),hf["tokenizer"]);handoffs={seed:audit_handoff(Path(getattr(a,seed+"_handoff")),seed) for seed in EXPECTED_ZIP_SHA256};outputs={}
