...
GEOMETRY_FREEZE_COMMIT = "9803aab542d08a257511cf409bf292428b11e0f8"
DISCOVERY_FREEZE_COMMIT = "22e0b01a2eaf5ff9dec34c3e19b51e288db9267c"

DISCOVERY_SELECTION_PATH = Path(
    "reports/reason_router_gen4_mamba370m_discovery_runs/"
    "g4k-mamba370-discovery-xg1-3001-3300-2gpu-2f9f5ce/"
...
SHARDS = (
    {
        ...
        "start_index": 0,
        "end_index": 150,
        "pair_first": "xg1_fact_3301",
        "pair_last": "xg1_fact_3450",
        "pair_count": 150,
    ...
    },
    {
        ...
        "start_index": 150,
        "end_index": 300,
        "pair_first": "xg1_fact_3451",
        "pair_last": "xg1_fact_3600",
        "pair_count": 150,
        "forward_budget": FORWARDS_PER_SHARD,
    },
)
...
ALPHA = 0.05
HYPOTHESIS_ORDER = ("H_DOM", "H_RES")
FAILURE_LABEL = "CROSS_BACKBONE_QUALITATIVE_RECURRENCE_NOT_ESTABLISHED"
SUCCESS_LABEL = "CROSS_BACKBONE_QUALITATIVE_RECURRENCE_ESTABLISHED"
...
RESULT_PASS = "PASS_MAMBA370M_CONFIRMATION"
ITEM_SCHEMA = "gen4-mamba370m-confirmation-item-v1"
INFERENCE_SCHEMA = "gen4-mamba370m-confirmation-inference-v1"
MANIFEST_SCHEMA = "gen4-mamba370m-confirmation-manifest-v1"
...
def validate_protocol() -> None:
    ...
    require(RESIDUAL_PLANES == ("P1", "P2", "P4", "P5"), "RESIDUAL_PLANES")
    require(FORWARDS_PER_CONDITION == 40, "FORWARDS_PER_CONDITION")
    require(FORWARDS_PER_PAIR == 160, "FORWARDS_PER_PAIR")
    require(TOTAL_FORWARD_BUDGET == 48000, "TOTAL_FORWARD_BUDGET")
    require(FORWARDS_PER_SHARD == 24000, "FORWARDS_PER_SHARD")
    require(len(SHARDS) == GPU_COUNT == 2, "SHARD_COUNT")
    require(HYPOTHESIS_ORDER == ("H_DOM", "H_RES"), "HYPOTHESIS_ORDER")
...
def load_frozen_selection() -> dict[str, Any]:
    ...
    require(
        selection["result"] == "PASS_MAMBA370M_DISCOVERY_SELECTION_FREEZE",
        "DISCOVERY_SELECTION_RESULT",
    )
    require(
        selection["selected_dominant_candidate"] == SELECTED_PLANE,
        "DISCOVERY_SELECTED_PLANE",
    )
    require(
        selection["response_blind_control_plane"] == CONTROL_PLANE,
        "DISCOVERY_CONTROL_PLANE",
    )
    ...
    require(
        selection["control_selection_uses_response"] is False,
        "DISCOVERY_CONTROL_RESPONSE",
    )
    require(
        selection["confirmation_accessed"] is False,
        "DISCOVERY_CONFIRMATION_ALREADY_ACCESSED",
    )
    require(
        selection["confirmation_response_accessed"] is False,
        "DISCOVERY_CONFIRMATION_RESPONSE_ALREADY_ACCESSED",
    )
    require(
        selection["scientific_conclusion_established"] is False,
        "DISCOVERY_CONCLUSION_PREMATURE",
    )
    return selection
...
def load_confirmation_population() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    require(
        sha256_file(source) == CONFIRMATION_SOURCE_SHA256,
        "CONFIRMATION_SOURCE_SHA",
    )
    require(
        sha256_file(rows) == CONFIRMATION_ROWS_SHA256,
        "CONFIRMATION_ROWS_SHA",
    )
    require(
        sha256_file(manifest_path) == CONFIRMATION_MANIFEST_SHA256,
    ...
    require(
        manifest["result"]
        == "PASS_MAMBA370M_XG1_CONFIRMATION_3301_3600_STRUCTURAL",
        "CONFIRMATION_RESULT",
    )
    ...
    require(
        manifest["selection_response_access_allowed"] is False,
        "CONFIRMATION_SELECTION_RESPONSE_BOUNDARY",
    )
...
def condition_correction(
    ...
    *,
    ...
    if condition == "dominant_restored":
        ...
        restoration_round_trip = float(
            torch.max(torch.abs(correction)).item()
        )
        require(
            restoration_round_trip <= TOL,
            f"DOMINANT_RESTORATION_ROUND_TRIP:{restoration_round_trip}",
        )
        ...
        require(
            ...
            f"DOMINANT_MATCHED_NORM:{matched_norm_mismatch}",
        )
        correction = (
            -selected_component
            + control_component
    ...
    elif condition == "residual_native":
        correction = torch.zeros(DIM, dtype=torch.float64)

    else:
        residual_component = torch.zeros(DIM, dtype=torch.float64)
        for plane in RESIDUAL_PLANES:
            residual_component += plane_component(
                value,
                plane,
                planes,
            )["component"]
        correction = (-residual_component).contiguous()
    ...
    native_selected = [
        float(torch.dot(value, selected_plus).item()),
        float(torch.dot(value, selected_minus).item()),
    ]
    post_selected = [
        float(torch.dot(post, selected_plus).item()),
        float(torch.dot(post, selected_minus).item()),
    ]

    if condition == "dominant_control":
        selected_post_max = max(
            abs(post_selected[0]),
            abs(post_selected[1]),
        )
        require(
            selected_post_max <= TOL,
            f"DOMINANT_SELECTED_NEUTRALIZATION:{selected_post_max}",
        )

    if condition == "residual_neutralized":
        selected_drift = max(
            abs(post_selected[0] - native_selected[0]),
            abs(post_selected[1] - native_selected[1]),
        )
        require(
            selected_drift <= TOL,
            f"RESIDUAL_SELECTED_DRIFT:{selected_drift}",
        )
        ...
        residual_post_max = max(abs(v) for v in residual_values)
        require(
            residual_post_max <= TOL,
            f"RESIDUAL_NEUTRALIZATION:{residual_post_max}",
        )
    ...
    return {
        ...
        "residual_post_max_abs_projection": residual_post_max,
        "selected_coefficient_drift_max_abs": selected_drift,
...
def run_pair(
    ...
    **kwargs: Any,
    ...
    q_native = float(conditions["residual_native"]["Q"])
    q_residual_neutralized = float(
        conditions["residual_neutralized"]["Q"]
    )

    d_dom = q_restored - q_control
    s_res = q_native - q_residual_neutralized
    ...
    return {
        ...
        "Q_control": q_control,
        "Q_native": q_native,
        "Q_residual_neutralized": q_residual_neutralized,
        "D_DOM": d_dom,
        "S_RES": s_res,
...
def holm_adjust(
    ...
    require(
        tuple(p_values.keys()) == HYPOTHESIS_ORDER,
        f"HOLM_ORDER:{tuple(p_values.keys())}",
    )
    m = len(p_values)
    ordered = sorted(
        p_values.items(),
    ...
    running = 0.0
    for rank_zero, (name, raw) in enumerate(ordered):
        multiplier = m - rank_zero
        candidate = min(1.0, multiplier * float(raw))
        running = max(running, candidate)
        adjusted_sorted.append((name, float(raw), running))

    adjusted_map = {
        name: {
            "p_raw": raw,
            "p_holm": adjusted,
            "reject_h0": adjusted <= ALPHA,
        }
        for name, raw, adjusted in adjusted_sorted
    }

    return {
        name: adjusted_map[name]
        for name in HYPOTHESIS_ORDER
    }


def infer_confirmation(
    items: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    require(len(items) == PAIR_COUNT, "INFERENCE_ITEM_COUNT")

    d_dom = [float(item["D_DOM"]) for item in items]
    s_res = [float(item["S_RES"]) for item in items]

    dom = one_sample_greater(d_dom)
    res = one_sample_greater(s_res)

    holm = holm_adjust({
        "H_DOM": float(dom["p_raw"]),
        "H_RES": float(res["p_raw"]),
    })

    both_reject = bool(
        holm["H_DOM"]["reject_h0"]
        and holm["H_RES"]["reject_h0"]
    )
    ...
    return {
        ...
        "H_DOM": {
            "definition": "D_DOM=Q_restored-Q_control",
            "null": "E[D_DOM] <= 0",
            "alternative": "E[D_DOM] > 0",
            **dom,
            **holm["H_DOM"],
        },
        "H_RES": {
            "definition":
                "S_RES=Q_native-Q_residual_neutralized",
            "null": "E[S_RES] <= 0",
            "alternative": "E[S_RES] > 0",
            **res,
            **holm["H_RES"],
        },
        "both_holm_reject": both_reject,
        "scientific_conclusion": conclusion,
        "claim_text": (
            "recurrence of a dominant component together with "
            "nontrivial residual causal structure"
            if both_reject
            else "cross-backbone qualitative recurrence not established"
        ),
        "selected_dominant_candidate": SELECTED_PLANE,
        "response_blind_control_plane": CONTROL_PLANE,
...
def worker_run(
    *,
    ...
    try:
        ...
        meta = {
            "schema_version": "gen4-mamba370m-confirmation-worker-v1",
            "shard_id": shard_id,
    ...
    *,
    ...
    inference.update({
        ...
        "confirmation_population": {
            ...
            "source_sha256": CONFIRMATION_SOURCE_SHA256,
            "rows_sha256": CONFIRMATION_ROWS_SHA256,
            "structural_manifest_sha256":
                CONFIRMATION_MANIFEST_SHA256,
        },
        "geometry_full_plane_gram_max_abs_residual":
            frozen["full_plane_gram_max_abs_residual"],
        "scipy_version": scipy.__version__,
        "discovery_selection_execution_head":
            selection["execution_head"],
    })
    ...
    (output_dir / INFERENCE_FILE).write_bytes(
        pretty_json_bytes(inference)
    )

    file_hashes = {
        ITEM_FILE: sha256_file(output_dir / ITEM_FILE),
        INFERENCE_FILE: sha256_file(output_dir / INFERENCE_FILE),
    }
    ...
    manifest = {
        ...
        "multiplicity": "Holm",
        "alpha_family": ALPHA,
        "both_holm_reject": bool(inference["both_holm_reject"]),
        "scientific_conclusion":
            inference["scientific_conclusion"],
        "selection_reopened": False,
        "rescue_performed": False,
...
def run_confirmation(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="gen4_mamba370m_confirmation_"
    ) as tmp:
        temp_dir = Path(tmp)
...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Mamba-370M prospective confirmation on XG1 3301..3600. "
            "Uses frozen discovery P3/P5 selection and tests exactly "
            "D_DOM and S_RES with a two-hypothesis Holm family."
        )
    )
...
def main(
    ...
    print(
        "D_DOM_MEAN="
        + format(float(inference["H_DOM"]["mean"]), ".17g")
    )
    print(
        "D_DOM_SD="
        + format(float(inference["H_DOM"]["sd"]), ".17g")
    )
    print(
        "D_DOM_T="
        + format(float(inference["H_DOM"]["t_statistic"]), ".17g")
    )
    print(
        "D_DOM_P_RAW="
        + format(float(inference["H_DOM"]["p_raw"]), ".17g")
    )
    print(
        "D_DOM_P_HOLM="
        + format(float(inference["H_DOM"]["p_holm"]), ".17g")
    )
    print(
        "D_DOM_REJECT="
        + str(bool(inference["H_DOM"]["reject_h0"]))
    )

    print(
        "S_RES_MEAN="
        + format(float(inference["H_RES"]["mean"]), ".17g")
    )
    print(
        "S_RES_SD="
        + format(float(inference["H_RES"]["sd"]), ".17g")
    )
    print(
        "S_RES_T="
        + format(float(inference["H_RES"]["t_statistic"]), ".17g")
    )
    print(
        "S_RES_P_RAW="
        + format(float(inference["H_RES"]["p_raw"]), ".17g")
    )
    print(
        "S_RES_P_HOLM="
        + format(float(inference["H_RES"]["p_holm"]), ".17g")
    )
    print(
        "S_RES_REJECT="
        + str(bool(inference["H_RES"]["reject_h0"]))
    )

    print(
        "BOTH_HOLM_REJECT="
        + str(bool(inference["both_holm_reject"]))
    )
    print(
        "SCIENTIFIC_CONCLUSION="
        + str(inference["scientific_conclusion"])
    )
    print(
        "SCIENTIFIC_MODEL_FORWARD_COUNT_THIS_RUN="
        + str(TOTAL_FORWARD_BUDGET)
    )
    print("CONFIRMATION_RANGE=xg1_fact_3301..xg1_fact_3600")
