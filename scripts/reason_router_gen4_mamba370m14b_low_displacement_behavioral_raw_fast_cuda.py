...
REQUIRED_ANCESTOR = "aa3b06fb95c8b2409ecf076dc252388ec13ea3c6"
...
BEHAVIOR_ITEM_FILE = "low_displacement_behavioral_items.jsonl"
...
BEHAVIOR_ITEM_SCHEMA = "gen4-mamba370m14b-low-displacement-behavior-row-v1"
BEHAVIOR_SUMMARY_SCHEMA = "gen4-mamba370m14b-low-displacement-behavior-summary-v1"
BEHAVIOR_MANIFEST_SCHEMA = "gen4-mamba370m14b-low-displacement-behavior-manifest-v1"
STATE_INDEX_SCHEMA = "gen4-mamba370m14b-low-displacement-state-index-v1"
STATE_MANIFEST_SCHEMA = "gen4-mamba370m14b-low-displacement-state-manifest-v1"

TECHNICAL_GATE_RESULT = "PASS_GEN4_LOW_DISPLACEMENT_BEHAVIORAL_TECHNICAL_GATE"
RAW_RESULT = "PASS_GEN4_LOW_DISPLACEMENT_BEHAVIORAL_RAW"
STATE_RESULT = "PASS_GEN4_LOW_DISPLACEMENT_STATE_CAPTURE_RAW"


class LowDisplacementRawError(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise LowDisplacementRawError(message)
...
def validate_protocol() -> None:
    ...
    require(ALPHAS == (0.5, 0.25), "ALPHAS")
    require(PRIMARY_ALPHA == 0.25, "PRIMARY_ALPHA")
    require(CONDITIONS == (
        "restored_native",
        "control_alpha_0_5",
        "control_alpha_0_25",
    ...
    require(FORWARDS_PER_ROW == 3, "FORWARDS_PER_ROW")
    require(FORWARDS_PER_SCALE == 1800, "FORWARDS_PER_SCALE")
    require(TOTAL_FORWARD_BUDGET == 3600, "TOTAL_FORWARD_BUDGET")
...
def load_population() -> tuple[
    ...
    require(manifest["behavioral_alphas"] == [0.5, 0.25], "HOLDOUT_ALPHAS")
    require(manifest["primary_alpha"] == 0.25, "HOLDOUT_PRIMARY_ALPHA")
...
def validate_behavioral_rows(
    ...
    for pair in PAIR_IDS:
        for cell in TARGET_CELLS:
            require((pair, cell) in by_key, f"TARGET_ROW_MISSING:{pair}:{cell}")
...
def build_input_state(
    *,
    ...
    for row in selected:
        ...
        name = by_name["A_NAME"]
        require(bool(identity["post4_eligible"]), f"IDENTITY_INELIGIBLE:{pair}:{cell}")
        require(bool(name["post4_eligible"]), f"NAME_INELIGIBLE:{pair}:{cell}")
        require(
        ...
        for anchor_name in ("A_IDENTITY", "A_NAME"):
            ...
            events[key] = by_name[anchor_name]
...
def run_three_forward_row(
    *,
    ...
    gate = {
        ...
        "three_forward_execution_pass": True,
        "alpha_0_5_scaling_pass":
            max_scaling_residual <= float(
                confirmation.transport_runtime.RUNTIME_CAST_TOL
            ),
        "alpha_0_25_scaling_pass":
            max_scaling_residual <= float(
                confirmation.transport_runtime.RUNTIME_CAST_TOL
            ),
        "native_replay_pass":
...
def technical_gate_worker(
    *,
    ...
    try:
        ...
        _items, _arrays, _capture_meta, gate = run_three_forward_row(
            spec=spec,
            model=model,
    ...
    try:
        ...
        for pair in PAIR_IDS:
            for cell in TARGET_CELLS:
                ...
                items, arrays, cmeta, gate = run_three_forward_row(
                    spec=spec,
                    model=model,
                    runtime_ctx=runtime_ctx,
                    frozen=frozen,
...
def run_technical_gate(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="gen4_low_displacement_gate_"
    ) as tmp:
        temp_dir = Path(tmp)
        _run_two_processes(
    ...
    print("MAMBA14B=PASS")
    print("THREE_FORWARD_TECHNICAL_ROW_PER_SCALE=PASS")
...
def run_raw(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="gen4_low_displacement_raw_"
    ) as tmp:
        temp_dir = Path(tmp)
        _run_two_processes(
            target=raw_worker,
    ...
    behavior_summary = {
        ...
        "scales": list(SCALES),
        "behavioral_alphas": list(ALPHAS),
        "primary_alpha_reserved_for_static_analysis": PRIMARY_ALPHA,
    ...
    print("BEHAVIOR_ROW_COUNT=3600")
    print("STATE_CAPTURE_ROW_COUNT=1200")
    print("MAMBA370M_FORWARD_COUNT=1800")
    print("MAMBA14B_FORWARD_COUNT=1800")
    print("SCIENTIFIC_FULL_MODEL_FORWARD_COUNT_TOTAL=3600")
...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Gen4 Mamba-370M/1.4B low-displacement behavioral raw runner. "
            "Technical-gate mode retains no numeric behavioral result. "
            "Raw mode performs exactly 3600 full-model forwards and no backward."
        )
    )
    parser.add_argument(
        "--mode",
