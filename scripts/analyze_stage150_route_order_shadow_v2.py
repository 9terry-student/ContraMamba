"""Stage150-A: route/order shadow analyzer v2.

This script evaluates ``route_order_reversal_v2`` as a diagnostic-only shadow
...
STAGE = "Stage150-A"
POLICY_NAME_DEFAULT = "route_order_reversal_v2"
...
BARE_TO_PATTERNS = [
    ("bare_to", re.compile(rf"\b{endpoint_pattern('src')}\s+to\s+{endpoint_pattern('dst')}\b")),
...
TITLE_CONTEXT_RE = re.compile(
    r"\b(?:song|single|album|studio\s+album|track|film|movie|episode|book|novel|"
    r"released|recorded\s+by|singer|artist|title|titled|called|named)\b",
    flags=re.IGNORECASE,
)
ROUTE_CONTEXT_RE = re.compile(
    r"\b(?:route|routes|travel|travels|traveled|travelled|trip|journey|flight|train|bus|"
    r"drive|drove|driving|walk|walked|sail|sailed|ship|shipped|moved|commute|commuted)\b",
    flags=re.IGNORECASE,
)
POLICY_INPUT_SAFETY = {
    ...
    "uses_deterministic_self_loop_guard": True,
    "uses_deterministic_title_context_guard": True,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the Stage150-A route/order shadow analyzer.")
    parser.add_argument("--input-jsonl", action="append", required=True, help="Prediction JSONL file. May repeat.")
    parser.add_argument("--output-dir", required=True, help="Directory for Stage150-A outputs.")
    ...
    parser.add_argument("--enable-bare-to-patterns", action="store_true", help="Enable conservative bare A to B route extraction.")
    parser.add_argument("--disable-title-context-guard", action="store_true", help="Disable title/media context candidate blocking.")
    ...
    with path.open("r", encoding="utf-8") as fh:
        for line_no, line in enumerate(fh, start=1):
            ...
            if not isinstance(row, dict):
                ...
                continue
            row["_stage150_source_file"] = str(path)
            row["_stage150_line_number"] = line_no
            rows.append(row)
    return rows, errors
...
def is_self_loop_route(source: str, destination: str, *, use_aliases: bool = True) -> bool:
    src = canonicalize_endpoint(source, use_aliases=use_aliases)
    dst = canonicalize_endpoint(destination, use_aliases=use_aliases)
    return bool(src and dst and src == dst)


def _span_has_quote_boundary(text: str, start: int, end: int) -> bool:
    left = text[max(0, start - 4) : start].strip()
    right = text[end : min(len(text), end + 4)].strip()
    quote_left = left.endswith(("'", '"', "''", "``", "`"))
    quote_right = right.startswith(("'", '"', "''", "''", "`"))
    return quote_left and quote_right


def is_title_like_route_context(text: Any, start: int, end: int) -> bool:
    text_s = "" if text is None else str(text)
    if _span_has_quote_boundary(text_s, start, end):
        return True
    window = text_s[max(0, start - 80) : min(len(text_s), end + 100)]
    return bool(TITLE_CONTEXT_RE.search(window))


def _bare_route_context_allowed(text: str, start: int, end: int, source: str, destination: str, *, use_aliases: bool) -> bool:
    window = text[max(0, start - 80) : min(len(text), end + 80)]
    if ROUTE_CONTEXT_RE.search(window):
        return True
    src = canonicalize_endpoint(source, use_aliases=use_aliases)
    dst = canonicalize_endpoint(destination, use_aliases=use_aliases)
    return bool(src in KNOWN_CANONICAL_ENDPOINTS and dst in KNOWN_CANONICAL_ENDPOINTS and src != dst)


def _append_rejected(rejected: list[dict[str, str]], source: str, destination: str, pattern: str, reason: str) -> None:
    rejected.append({"source": source, "destination": destination, "pattern": pattern, "reason": reason})


def extract_directional_routes_v2(
    text: Any,
    *,
    use_aliases: bool = True,
    enable_bare_to_patterns: bool = False,
    title_context_guard_enabled: bool = True,
) -> dict[str, Any]:
    if text is None:
        return {"routes": [], "self_loop_routes_rejected": [], "title_like_routes_rejected": []}
    routes: list[dict[str, str]] = []
    self_loop_rejected: list[dict[str, str]] = []
    title_like_rejected: list[dict[str, str]] = []
    seen: set[tuple[str, str, str]] = set()
    text_s = str(text)
    occupied_explicit_spans: list[tuple[int, int]] = []

    def consider(pattern_name: str, match: re.Match[str], *, active: bool, bare: bool) -> None:
        if bare and any(start <= match.start() and match.end() <= end for start, end in occupied_explicit_spans):
            return
        src = normalize_endpoint_span(match.group("src"))
        dst = normalize_endpoint_span(match.group("dst"))
        if not src or not dst:
            return
        title_like = title_context_guard_enabled and is_title_like_route_context(text_s, match.start(), match.end())
        self_loop = is_self_loop_route(src, dst, use_aliases=use_aliases)
        if self_loop:
            _append_rejected(self_loop_rejected, src, dst, pattern_name, "canonical_source_equals_destination")
        if title_like:
            _append_rejected(title_like_rejected, src, dst, pattern_name, "title_or_media_context")
        if self_loop or title_like:
            return
        if bare and not active:
            return
        if bare and not _bare_route_context_allowed(text_s, match.start(), match.end(), src, dst, use_aliases=use_aliases):
            return
        ...
        seen.add(key)
        if not bare:
            occupied_explicit_spans.append(match.span())

    for pattern_name, regex in DIRECTIONAL_PATTERNS:
        for match in regex.finditer(text_s):
            consider(pattern_name, match, active=True, bare=False)
    for pattern_name, regex in BARE_TO_PATTERNS:
        for match in regex.finditer(text_s):
            consider(pattern_name, match, active=enable_bare_to_patterns, bare=True)
    return {
        "routes": routes,
        "self_loop_routes_rejected": self_loop_rejected,
        "title_like_routes_rejected": title_like_rejected,
    }


def extract_directional_routes(text: Any) -> list[dict[str, str]]:
    return extract_directional_routes_v2(text)["routes"]
...
def compute_route_order_features_v2(
    claim_text: Any,
    evidence_text: Any,
    *,
    use_aliases: bool = True,
    ...
    claim_extraction = extract_directional_routes_v2(
        claim_text,
        use_aliases=use_aliases,
        enable_bare_to_patterns=enable_bare_to_patterns,
        title_context_guard_enabled=title_context_guard_enabled,
    )
    evidence_extraction = extract_directional_routes_v2(
        evidence_text,
        use_aliases=use_aliases,
        enable_bare_to_patterns=enable_bare_to_patterns,
        title_context_guard_enabled=title_context_guard_enabled,
    )
    claim_routes = claim_extraction["routes"]
    evidence_routes = evidence_extraction["routes"]
    ...
    return {
        ...
        "self_loop_routes_rejected": claim_extraction["self_loop_routes_rejected"] + evidence_extraction["self_loop_routes_rejected"],
        "title_like_routes_rejected": claim_extraction["title_like_routes_rejected"] + evidence_extraction["title_like_routes_rejected"],
    }
...
def compute_route_order_features(
    ...
    *,
    ...
    return compute_route_order_features_v2(
        claim_text,
        evidence_text,
        use_aliases=use_aliases,
...
def apply_route_order_shadow_policy_v2(
    prediction: str | None,
    claim_text: Any,
    evidence_text: Any,
    *,
    ...
    features = compute_route_order_features_v2(
        claim_text,
        evidence_text,
        use_aliases=use_aliases,
        organization_block_enabled=organization_block_enabled,
    ...
    return {
        ...
        "stage150_original_prediction": prediction,
        "stage150_policy": policy_name,
        "stage150_policy_triggered": triggered,
        "stage150_claim_routes_raw": features["claim_routes_raw"],
        "stage150_evidence_routes_raw": features["evidence_routes_raw"],
        "stage150_claim_routes_canonical": features["claim_routes_canonical"],
        "stage150_evidence_routes_canonical": features["evidence_routes_canonical"],
        "stage150_claim_nondirectional_routes": features["claim_nondirectional_routes"],
        "stage150_evidence_nondirectional_routes": features["evidence_nondirectional_routes"],
        "stage150_route_reversal_detected": features["route_reversal_detected"],
        "stage150_trigger_blocked_by_org_like_endpoint": features["trigger_blocked_by_org_like_endpoint"],
        "stage150_claim_org_like_endpoints": features["claim_org_like_endpoints"],
        "stage150_evidence_org_like_endpoints": features["evidence_org_like_endpoints"],
        "stage150_self_loop_routes_rejected": features["self_loop_routes_rejected"],
        "stage150_title_like_routes_rejected": features["title_like_routes_rejected"],
        "stage150_bare_to_patterns_enabled": enable_bare_to_patterns,
        "stage150_diagnostic_only": True,
    }
...
def apply_route_order_shadow_policy(
    ...
    *,
    ...
    return apply_route_order_shadow_policy_v2(
        prediction,
        claim_text,
        evidence_text,
...
def compute_metrics(
    ...
    for row in rows:
        ...
        if row.get("stage150_claim_routes_canonical"):
            counters["rows_claim_directional_route_present"] += 1
        if row.get("stage150_evidence_routes_canonical"):
            counters["rows_evidence_directional_route_present"] += 1
        if row.get("stage150_claim_routes_canonical") and row.get("stage150_evidence_routes_canonical"):
            counters["rows_both_directional_route_present"] += 1
        if row.get("stage150_claim_nondirectional_routes"):
            counters["rows_claim_nondirectional_route_present"] += 1
        if row.get("stage150_evidence_nondirectional_routes"):
            counters["rows_evidence_nondirectional_route_present"] += 1
        if pred == "SUPPORT" and row.get("stage150_route_reversal_detected"):
            counters["support_rows_with_route_reversal"] += 1
        if pred == "SUPPORT" and row.get("stage150_trigger_blocked_by_org_like_endpoint"):
            counters["support_rows_blocked_by_org_like_endpoint"] += 1
        if row.get("stage150_claim_org_like_endpoints") or row.get("stage150_evidence_org_like_endpoints"):
            counters["rows_org_like_endpoint_present"] += 1
        self_loop_rejected = row.get("stage150_self_loop_routes_rejected") or []
        title_like_rejected = row.get("stage150_title_like_routes_rejected") or []
        if self_loop_rejected:
            counters["rows_self_loop_routes_rejected"] += 1
            counters["total_self_loop_routes_rejected"] += len(self_loop_rejected)
        if title_like_rejected:
            counters["rows_title_like_routes_rejected"] += 1
            counters["total_title_like_routes_rejected"] += len(title_like_rejected)
        ...
        if gold in label_set and pred in label_set and after in label_set:
            ...
            if pred == "SUPPORT":
                triggered = bool(row.get("stage150_policy_triggered"))
    ...
    metrics: dict[str, Any] = {
        ...
        "rows_self_loop_routes_rejected": counters["rows_self_loop_routes_rejected"],
        "rows_title_like_routes_rejected": counters["rows_title_like_routes_rejected"],
        "total_self_loop_routes_rejected": counters["total_self_loop_routes_rejected"],
        "total_title_like_routes_rejected": counters["total_title_like_routes_rejected"],
...
def add_policy_fields(
    ...
    for row in rows:
        ...
        out = dict(row)
        out["_stage150_normalized_prediction"] = pred
        out["_stage150_normalized_gold"] = gold
        out.update(
            apply_route_order_shadow_policy(
                ...
                enable_bare_to_patterns=args.enable_bare_to_patterns,
                title_context_guard_enabled=not args.disable_title_context_guard,
            )
        )
        enriched.append(out)
    return enriched
...
def process_file(path: Path, args: argparse.Namespace, label_set: list[str]) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    changed = [row for row in enriched if row.get("stage150_policy_triggered")]
...
def aggregate_metrics(rows: list[dict[str, Any]], file_metrics: list[dict[str, Any]], label_set: list[str], malformed_count: int) -> dict[str, Any]:
    aggregate = compute_metrics(rows, "_stage150_normalized_prediction", "_stage150_normalized_gold", None, None, label_set)
...
def choose_decision(aggregate: dict[str, Any]) -> str:
    if aggregate.get("n_valid_rows", 0) == 0:
        return "STAGE150_ROUTE_ORDER_SHADOW_V2_NO_VALID_INPUTS"
    if aggregate.get("n_files_with_usable_gold", 0) == 0:
        return "STAGE150_ROUTE_ORDER_SHADOW_V2_COUNT_ONLY_NO_GOLD"
    ...
    if delta_false_support is not None and delta_false_support < 0 and delta_false_ne == 0 and min_delta_macro_f1 is not None and min_delta_macro_f1 >= 0:
        return "STAGE150_ROUTE_ORDER_SHADOW_V2_CANDIDATE_PASS"
    if delta_false_support is not None and delta_false_support < 0 and delta_false_ne is not None and delta_false_ne <= 3 and min_delta_macro_f1 is not None and min_delta_macro_f1 >= -0.01:
        return "STAGE150_ROUTE_ORDER_SHADOW_V2_CANDIDATE_MILD_TRADEOFF"
    if aggregate.get("n_changed_total", 0) == 0:
        return "STAGE150_ROUTE_ORDER_SHADOW_V2_NO_EFFECT"
    if delta_false_support is not None and delta_false_support < 0:
        return "STAGE150_ROUTE_ORDER_SHADOW_V2_MIXED"
    return "STAGE150_ROUTE_ORDER_SHADOW_V2_HARM_OR_NO_FS_GAIN"
                ...
                out.append(
                    {
                        ...
                        "rows_self_loop_routes_rejected": metrics["rows_self_loop_routes_rejected"],
                        "rows_title_like_routes_rejected": metrics["rows_title_like_routes_rejected"],
                    }
                )
    return out
...
def build_markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Why v2 was needed",
        "",
        "Stage148-A exposed a broader-scan false positive where `Side to Side` was interpreted as a route. Stage150-A keeps Stage147 v1 untouched and adds a separate v2 shadow analyzer with self-loop and title/context guards.",
        "",
        "## Stage149 false-positive cause",
        "",
        "The phrase `Side to Side` is a song title, not a source/destination route. V1 could extract it as a degenerate `(side -> side)` self-loop, so v2 rejects canonical self-loops and title-like route contexts before reversal matching.",
        "",
        "## v2 policy definition",
        "",
        f"Policy `{report['policy']['name']}`: for original SUPPORT predictions only, extract conservative directional claim and evidence routes, canonicalize endpoints, reject self-loop and title-like candidates, and shadow SUPPORT to NOT_ENTITLED only when claim has `(A -> B)` and evidence has `(B -> A)` without an organization-like endpoint block.",
        "",
        "## Self-loop guard",
        "",
        "Any candidate whose canonical source equals canonical destination is dropped and counted in `stage150_self_loop_routes_rejected`.",
        "",
        "## Title/context guard",
        "",
        "Quoted or obvious media-title contexts near a candidate route phrase are dropped and counted in `stage150_title_like_routes_rejected`; the guard blocks candidates, not entire rows.",
        "",
        "## Bare `A to B` policy",
        "",
        "Bare `A to B` extraction is disabled by default. `--enable-bare-to-patterns` enables conservative extraction, while rejected bare candidates are still accounted for by the self-loop/title guards.",
        "",
        "## Policy input safety",
        "",
        "The policy uses only claim text, evidence text, the original prediction label, deterministic route extraction, deterministic aliases, deterministic self-loop rejection, deterministic title/context guards, and deterministic organization-like endpoint rules.",
        "",
        "```json",
        ...
        markdown_table(
            ...
            [
                ...
                "rows_self_loop_routes_rejected",
                "rows_title_like_routes_rejected",
                "total_self_loop_routes_rejected",
                "total_title_like_routes_rejected",
                "false_support_before",
                "false_support_after",
                "delta_false_support",
                "false_ne_before",
        ...
        "## Safety policy",
        "",
        "This script is shadow-only. It does not modify source predictions. It is separate from Stage147 v1. It must not be interpreted as final model integration. It was created because Stage148-A exposed a `Side to Side` self-loop/title false positive.",
        "",
        "```json",
...
def trim_output_row(row: dict[str, Any]) -> dict[str, Any]:
    internal = {"_stage150_source_file", "_stage150_line_number", "_stage150_normalized_prediction", "_stage150_normalized_gold"}
    out = {key: value for key, value in row.items() if key not in internal}
    out["stage150_source_file"] = row.get("_stage150_source_file")
    out["stage150_line_number"] = row.get("_stage150_line_number")
    return out
...
def main() -> None:
    ...
    file_metrics_path = output_dir / "stage150_file_metrics.csv"
    aggregate_metrics_path = output_dir / "stage150_aggregate_metrics.json"
    group_metrics_path = output_dir / "stage150_group_metrics.csv"
    changed_examples_path = output_dir / "stage150_changed_examples.jsonl"
    shadow_predictions_path = output_dir / "stage150_shadow_predictions.jsonl"
    report_json_path = output_dir / "stage150_route_order_shadow_v2_report.json"
    report_md_path = output_dir / "stage150_route_order_shadow_v2_report.md"
    ...
    interpretation = (
        "Stage150-A is a shadow-only route/order diagnostic for cases where claim and evidence share the same "
    ...
    route_extraction_summary = {
        "directional_patterns": ["from SOURCE to DEST", "from SOURCE into DEST", "from SOURCE toward DEST", "SOURCE -> DEST"],
        "nondirectional_patterns": ["between A and B"],
        "bare_to_patterns_enabled": args.enable_bare_to_patterns,
        "bare_to_patterns_default_enabled": False,
    ...
    report = {
        ...
        "v2_refinement_summary": {
            "created_because": "Stage148-A exposed a Side to Side self-loop/title false positive.",
            "stage147_v1_modified": False,
            "self_loop_candidates_rejected": True,
            "title_like_candidates_rejected": not args.disable_title_context_guard,
            "bare_to_patterns_default_enabled": False,
            "bare_to_patterns_enabled_this_run": args.enable_bare_to_patterns,
        },
