...
STATIC_COMMIT = "4a1d5871fad17a34951bc433a283c95e682a081b"
AUTHORITY_COMMIT = "25ff56c80ccd943bb386ebc7ab6612fe6e68d470"

AUTHORITY_PATH = Path(
    "reports/"
    "reason_router_gen4_pp3_excluded_residual_individual_plane_"
    "restoration_sufficiency_localization_implementation_authority.md"
)
AUTHORITY_BLOB = "894d665dc89f41435363f55d0f4eadc3a3b24402"
...
SOURCE_SHA = "d2cc7276254bb96c32ea54af3c3fb768cbe005dcd7cb329512232df18629ea81"
ROWS_SHA = "56957588464df337ad65509e377b534e49c264ddaac766e3131633dd5e2ac087"
STRUCT_SHA = "fd37beb2eb1d30c7af81d6f771ddfee731e47ccdb56b32ddf84cc57a922836e1"
ANCHOR_SHA = "bf2c0a2a82ecff43fa8bc1b9beb31e214563d027d50a3a24df4a1d488da9115f"
ELIG_SHA = "9c9ba1862837a3e7bbc0010636a9eafc1f5c8b438141dc04be2e902045ed6905"
GEOMETRY_SHA = "9e63a5ecc7cd172cd07335b36a637bf2ec6ba03569ebf02b9a6cce80f2fab51f"
PREPARATION_SHA = "afe37f32950724c2a997a233dba9ff456705d646ab01f5f69487ecaeef136574"
...
ITEM_FILE = "pp3_excluded_residual_individual_plane_restoration_sufficiency_items.jsonl"
SUMMARY_FILE = "pp3_excluded_residual_individual_plane_restoration_sufficiency_summary.json"
MANIFEST_FILE = "artifact_manifest.json"
CHECKSUM_FILE = "SHA256SUMS.txt"

ITEM_SCHEMA = "gen4-pp3-excluded-residual-individual-plane-restoration-sufficiency-item-v1"
SUMMARY_SCHEMA = "gen4-pp3-excluded-residual-individual-plane-restoration-sufficiency-summary-v1"
MANIFEST_SCHEMA = "gen4-pp3-excluded-residual-individual-plane-restoration-sufficiency-manifest-v1"
RESULT_PASS = (
    "PASS_PP3_EXCLUDED_RESIDUAL_INDIVIDUAL_PLANE_RESTORATION_SUFFICIENCY_"
...
class ResidualIndividualPlaneRestorationSufficiencyError(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise ResidualIndividualPlaneRestorationSufficiencyError(message)
...
def git(*args: str) -> str:
    try:
        ...
        raise ResidualIndividualPlaneRestorationSufficiencyError(
...
def replacement_condition(plane: str) -> str:
    return f"{plane.lower()}_quarter_turn_replacement"
...
def parse_condition(condition: str) -> tuple[str | None, str]:
    ...
    for plane in RESIDUAL_PLANES:
        ...
        if condition == replacement_condition(plane):
            return plane, "quarter_turn_replacement"
    raise ResidualIndividualPlaneRestorationSufficiencyError(
...
def validate_static_inputs() -> dict[str, Any]:
    checks = {
        ...
        STATIC_SCRIPT_PATH: STATIC_SCRIPT_SHA,
    }
    ...
    require(
        structural["pair_id_first"] == "xg1_fact_2101"
        and structural["pair_id_last"] == "xg1_fact_2400"
        and structural["source_pair_count"] == N
        and structural["row_count"] == ROWS
        and structural["pair_id_overlap_with_001_2100"] == 0
        and structural["claim_overlap_with_001_2100"] == 0
        and structural["evidence_overlap_with_001_2100"] == 0
        and structural["claim_evidence_row_overlap_with_001_2100"] == 0
    ...
    require(
        ...
        and preparation["population"]["pair_id_first"] == "xg1_fact_2101"
        and preparation["population"]["pair_id_last"] == "xg1_fact_2400"
    ...
    require(
        ...
        and eligibility["pair_id_first"] == "xg1_fact_2101"
        and eligibility["pair_id_last"] == "xg1_fact_2400"
    ...
    for plane in RESIDUAL_PLANES:
        replacement = geometry[
            "per_plane_quarter_turn_replacement_component"
        ][plane]
        require(
            float(replacement["max_abs_l2_mismatch"]) <= TOL
            and abs(float(
                replacement["max_abs_treatment_control_dot"]
            )) <= TOL
            and abs(float(
                replacement[
            ...
            and abs(float(
                replacement["max_abs_projection_onto_pp3"]
            )) <= TOL
            and replacement["equal_norm_identity"] is True
            and replacement["orthogonal_control_identity"] is True
            and replacement["response_guided_weighting"] is False
            and replacement["response_guided_plane_selection"] is False,
            f"GEOMETRY_REPLACEMENT:{plane}",
        )

        audit = geometry["per_plane_restoration_audit"][plane]
        for key in (
            "coefficient_recovery_max_abs_residual",
            "neutralized_target_projection_max_abs",
            ...
            require(
                ...
                f"GEOMETRY_RESTORATION:{plane}:{key}",
            )
        require(
            audit["response_guided_selection"] is False
            and audit["response_guided_weighting"] is False,
            f"GEOMETRY_RESPONSE_GUIDED:{plane}",
        )
...
def all_plane_components(
    ...
    for plane in RESIDUAL_PLANES:
        ...
        require(
            mismatch <= TOL,
            f"RESTORATION_REPLACEMENT_L2:{plane}:{mismatch}",
        )
        require(
            abs(dot) <= TOL,
            f"NATIVE_QUARTER_DOT:{plane}:{dot}",
        )
...
def condition_correction(
    ...
    target_neutral = None
    replacement_coordinate_residual = None

    if kind == "neutralized":
        assert selected_plane is not None
        target_neutral = max(
            abs(value)
            for value in residual_post[selected_plane]
        )
        require(
            target_neutral <= TOL,
    ...
    if kind == "quarter_turn_replacement":
        assert selected_plane is not None
        a, b = info["coefficients"][selected_plane]
        expected = [-float(b), float(a)]
        observed = residual_post[selected_plane]
        replacement_coordinate_residual = max(
            abs(float(observed[0]) - expected[0]),
            abs(float(observed[1]) - expected[1]),
        )
        require(
            replacement_coordinate_residual <= TOL,
            (
                f"REPLACEMENT_COORDINATES:{selected_plane}:"
                f"{replacement_coordinate_residual}"
            ),
        )
    ...
    if selected is None:
        ...
        component_l2 = selected["native_component_l2"]
        quarter_l2 = selected["quarter_turn_component_l2"]
        mismatch = selected[
            "restoration_replacement_addition_l2_mismatch"
        ]
        dot = selected["native_quarter_turn_dot"]
    ...
    return {
        ...
        "target_plane_neutralization_max_abs_projection":
            target_neutral,
        "replacement_target_coordinate_max_abs_residual":
            replacement_coordinate_residual,
    }
...
def apply_hook(
    ...
    *,
    ...
    audit.update({
        ...
        "restoration_replacement_addition_l2_mismatch":
            condition_info["restoration_replacement_addition_l2_mismatch"],
...
def endpoints_from_conditions(
    ...
    for plane in RESIDUAL_PLANES:
        plane_endpoints[plane] = plane_endpoint(
            ...
            float(by_condition[replacement_condition(plane)]["Q"]),
        )
                ...
                for role, branch_sign in (("tp", 1), ("tm", -1)):
                    ...
                    if selected_plane is None:
                        require(
                            ...
                            and audit["native_component_l2"] is None
                            and audit["quarter_turn_component_l2"] is None
                            and audit["restoration_replacement_addition_l2_mismatch"] is None
                            and audit["native_quarter_turn_dot"] is None
                            and audit[
                                "target_plane_neutralization_max_abs_projection"
                            ] is None
                            and audit[
                        ...
                        require(
                            float(audit["restoration_replacement_addition_l2_mismatch"])
                            <= TOL
                            and abs(float(
                        ...
                        if kind == "neutralized":
                            ...
                            require(
                                ...
                                and replacement_residual is not None
                                and float(replacement_residual) <= TOL,
                                "TARGET_REPLACEMENT_AUDIT",
                            )
...
def validate_artifact(
    ...
    require(
        ...
        and summary["pair_id_first"] == "xg1_fact_2101"
        and summary["pair_id_last"] == "xg1_fact_2400",
        "SUMMARY_POPULATION",
    )
    require(
    ...
    require(
        ...
        == "for each k in {P1,P2,P4,P5}: D_SUF_k=Q0-QC_k",
        "SUMMARY_ENDPOINT",
    )
    require(
...
def run_observation(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="gen4_residual_individual_plane_restoration_sufficiency_"
    ) as temp_name:
...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "individual-plane restoration sufficiency localization; no statistical inference."
        )
    )
