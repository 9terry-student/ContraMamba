...
REQUIRED_BASE_COMMIT = "9b0dfe91780de00347ce3827102f08210d6b558e"
...
ITEM_FILE = "factor2_small_alpha_native_readout_items.jsonl"
SUMMARY_FILE = "raw_readout_summary.json"
MANIFEST_FILE = "artifact_manifest.json"
CHECKSUM_FILE = "SHA256SUMS.txt"

ITEM_SCHEMA = "gen4-factor2-small-alpha-native-readout-row-v1"
SUMMARY_SCHEMA = "gen4-factor2-small-alpha-native-readout-summary-v1"
MANIFEST_SCHEMA = "gen4-factor2-small-alpha-native-readout-manifest-v1"

GATE_RESULT = "PASS_GEN4_FACTOR2_SMALL_ALPHA_NATIVE_READOUT_TECHNICAL_GATE"
RAW_RESULT = "PASS_GEN4_FACTOR2_SMALL_ALPHA_NATIVE_READOUT_RAW"
...
def validate_protocol() -> None:
    ...
    token_summary = ROOT / (
        ...
        "factor2_small_alpha_tokenizer_anchor_eligibility_v1/"
    ...
    require(
        gate["result"]
        == "PASS_GEN4_FACTOR2_SMALL_ALPHA_TOKENIZER_ANCHOR_ELIGIBILITY",
        "TOKEN_GATE_RESULT",
    )
    ...
    *,
    ...
    try:
        ...
        paths["gate"].write_bytes(pretty_json_bytes({
            ...
            "frozen_factor2_population_pass": True,
    ...
    *,
    ...
    for scale in SCALES:
        ...
        process = ctx.Process(
            target=target,
            kwargs=kwargs,
            name=f"factor2-smallalpha-native-readout-{scale}",
        )
        process.start()
...
def run_technical_gate(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="gen4_factor2_smallalpha_native_readout_gate_"
    ) as tmp:
        temp_dir = Path(tmp)
        run_two_processes(
        ...
        for scale in SCALES:
            ...
            for key in (
                ...
                "frozen_factor2_population_pass",
                "token_anchor_pass",
...
def run_raw(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="gen4_factor2_smallalpha_native_readout_raw_"
    ) as tmp:
        temp_dir = Path(tmp)
        run_two_processes(
            target=raw_worker,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Measure response-blind native local task-margin readout Delta_L "
            "on the frozen XG1 5701..6000 factor-2 small-alpha cohort, "
            "without reading any behavioral response artifact. No p-value."
        )
    )
    parser.add_argument(
        "--mode",
