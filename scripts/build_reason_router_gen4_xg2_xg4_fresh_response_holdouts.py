...
HOLDOUT_FIRST_PAIR = 301
HOLDOUT_LAST_PAIR = 600
...
class FreshResponseHoldoutBuildError(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise FreshResponseHoldoutBuildError(message)
...
def git(*args: str) -> str:
    try:
        ...
        raise FreshResponseHoldoutBuildError(
...
def authenticate_authority() -> None:
    ...
    require(
        observed_design_blob == DESIGN_BLOB,
        f"DESIGN_BLOB:{observed_design_blob}",
    )
    ...
    require(
        current_design_blob == DESIGN_BLOB,
        f"DESIGN_DRIFT:{current_design_blob}",
    )
...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "holdouts using absolute source indices 301-600. "
