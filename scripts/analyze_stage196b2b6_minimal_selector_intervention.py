...
def sources(ns:argparse.Namespace,g:list[dict[str,Any]])->dict[str,Any]:
 ...
 gate(g,"invocation","","explicit_paths",{"all_absolute_and_under_repo_root":True},{"all_absolute_and_under_repo_root":explicit},root.is_dir() and explicit,"all paths must be explicit and under repo root")
 ...
 for label,path,names in (("b2b5",b5p,B5_FILES),("b2b4",b4p,B4_FILES)):
  c=read_csv(path.parent/names[-1]);gate(g,label,"",f"{label}_contract_closure",{"failed":0},{"rows":len(c),"failed":sum(not boolean(r.get("passed"),"passed") for r in c)},closed(c),f"{label} contract failed")
...
def load_p0(ns:argparse.Namespace,src:dict[str,Any],g:list[dict[str,Any]])->dict[str,Any]:
 ...
 for run in RUNS:
  ...
  m=read_json(cd/mn);hashes[str(cd/mn)]=sha256(cd/mn);mok=m.get("completed") is True and m.get("current_git_commit")==P0_COMMIT and m.get("seed")==seed and m.get("gradient_ownership_mode")==mode and m.get("sidecar_files")==cn;gate(g,"p0",run,"composer_manifest",{"completed":True,"commit":P0_COMMIT,"seed":seed,"mode":mode},{"completed":m.get("completed"),"commit":m.get("current_git_commit"),"seed":m.get("seed"),"mode":m.get("gradient_ownership_mode")},mok,"manifest changed")
  schema=None
...
def candidates(src:dict[str,Any],p0:dict[str,Any],g:list[dict[str,Any]])->tuple[list[dict[str,Any]],dict[Any,Any],dict[Any,Any]]:
 ...
 minimal=[r for r,m in parsed if not any(om<m for _,om in parsed)];gate(g,"candidate","","candidate_inclusion_minimal_subset_closure",{"nonempty":True},{"nonempty":bool(minimal),"eligible":len(eligible),"retained":len(minimal),"masks":sorted(r["feature_subset_mask"] for r in minimal)},bool(minimal),"no minimal candidates")
...
def clean_dev(candidates:list[dict[str,Any]],maps:dict[Any,Any],ctx:dict[str,Any],p0:dict[str,Any],g:list[dict[str,Any]])->tuple[list[dict[str,Any]],list[dict[str,Any]],dict[str,Any],dict[str,Any]]:
 ...
 primary_cases_by_seed={seed:{key for key in ctx["meta"] if key[0]==seed} for seed in PRIMARY_SEEDS};discovery_identities_by_seed={seed:{(ctx["meta"][key]["id"],ctx["meta"][key]["source_row_id"],integer(ctx["meta"][key]["dev_position"],"position")) for key in primary_cases_by_seed[seed]} for seed in PRIMARY_SEEDS};discovery_identity_intersection=discovery_identities_by_seed[184]&discovery_identities_by_seed[185];discovery_identity_union=discovery_identities_by_seed[184]|discovery_identities_by_seed[185]
 accounting_required={"seed_conditioned_primary_cases":{"total":16,"seed184":11,"seed185":5},"discovery_data_identities":{"seed184":11,"seed185":5,"intersection":3,"union":13},"clean_dev_per_seed":{"all":720,"discovery_union":13,"nondiscovery":707}};accounting_observed={"seed_conditioned_primary_cases":{"total":len(ctx["meta"]),"seed184":len(primary_cases_by_seed[184]),"seed185":len(primary_cases_by_seed[185])},"discovery_data_identities":{"seed184":len(discovery_identities_by_seed[184]),"seed185":len(discovery_identities_by_seed[185]),"intersection":len(discovery_identity_intersection),"union":len(discovery_identity_union),"intersection_identities":[list(value) for value in sorted(discovery_identity_intersection)],"union_set_equals_seed_union":discovery_identity_union==discovery_identities_by_seed[184]|discovery_identities_by_seed[185],"intersection_set_equals_seed_intersection":discovery_identity_intersection==discovery_identities_by_seed[184]&discovery_identities_by_seed[185]},"clean_dev_per_seed":{"all":720,"discovery_union":len(discovery_identity_union),"nondiscovery":720-len(discovery_identity_union)}};accounting_passed=(len(ctx["meta"])==16 and {seed:len(primary_cases_by_seed[seed]) for seed in PRIMARY_SEEDS}=={184:11,185:5} and {seed:len(discovery_identities_by_seed[seed]) for seed in PRIMARY_SEEDS}=={184:11,185:5} and len(discovery_identity_intersection)==3 and len(discovery_identity_union)==13 and accounting_observed["discovery_data_identities"]["union_set_equals_seed_union"] and accounting_observed["discovery_data_identities"]["intersection_set_equals_seed_intersection"]);gate(g,"population","","primary_case_and_discovery_identity_accounting",accounting_required,accounting_observed,accounting_passed,"primary-case/discovery-identity accounting failed")
 clean_identity_sets={seed:{ident[1:] for ident in by_ident if ident[0]==seed} for seed in SEEDS};clean_identity_counts={str(seed):len(clean_identity_sets[seed]) for seed in SEEDS};clean_identity_hashes={str(seed):hashlib.sha256(canon(sorted(clean_identity_sets[seed])).encode()).hexdigest() for seed in SEEDS};missing_discovery={str(seed):[list(value) for value in sorted(discovery_identity_union-clean_identity_sets[seed])] for seed in SEEDS};discovery_counts={str(seed):len(clean_identity_sets[seed]&discovery_identity_union) for seed in SEEDS};nondiscovery_counts={str(seed):len(clean_identity_sets[seed]-discovery_identity_union) for seed in SEEDS};duplicate_counts={str(seed):sum(1 for ident in by_ident if ident[0]==seed)-len(clean_identity_sets[seed]) for seed in SEEDS};identity_sets_equal=clean_identity_sets[183]==clean_identity_sets[184]==clean_identity_sets[185]
 clean_required={"seed_identity_counts":{"183":720,"184":720,"185":720},"same_identity_set":True,"discovery_union_occurrences_per_seed":{"183":13,"184":13,"185":13},"nondiscovery_per_seed":{"183":707,"184":707,"185":707},"missing_discovery_identities":{"183":[],"184":[],"185":[]},"duplicate_data_identities":{"183":0,"184":0,"185":0}};clean_observed={"seed_identity_counts":clean_identity_counts,"same_identity_set":identity_sets_equal,"identity_set_sha256":clean_identity_hashes,"discovery_union_occurrences_per_seed":discovery_counts,"nondiscovery_per_seed":nondiscovery_counts,"missing_discovery_identities":missing_discovery,"duplicate_data_identities":duplicate_counts};clean_identity_passed=(clean_identity_counts==clean_required["seed_identity_counts"] and identity_sets_equal and discovery_counts==clean_required["discovery_union_occurrences_per_seed"] and nondiscovery_counts==clean_required["nondiscovery_per_seed"] and missing_discovery==clean_required["missing_discovery_identities"] and duplicate_counts==clean_required["duplicate_data_identities"]);gate(g,"population","","cross_seed_clean_dev_identity_set_closure",clean_required,clean_observed,clean_identity_passed,"cross-seed clean-dev identity closure failed")
 population_semantics={"primary_case_count":16,"seed184_primary_case_count":11,"seed185_primary_case_count":5,"discovery_identity_union_count":13,"seed184_discovery_identity_count":11,"seed185_discovery_identity_count":5,"cross_seed_identity_intersection_count":3,"cross_seed_overlapping_data_identities":[list(value) for value in sorted(discovery_identity_intersection)],"nondiscovery_count_per_seed":707,"all_clean_dev_count_per_seed":720,"primary_case_key":["seed","stable_row_id"],"discovery_data_identity_key":["id","source_row_id","dev_position"],"safety_authorization_population":"NONDISCOVERY","case_identity_distinction":"16 is the seed-conditioned treatment-case count; 13 is the distinct clean-dev data-identity union; 16 is not a per-seed clean-dev row count."}
 ...
 for c in candidates:
  ...
  for ident,epochs in sorted(by_ident.items()):
   seed,identity,source,pos=ident;tail=[epochs[e]["joint"] for e in TAIL];sig=canon(signature(members,tail));signature_seen=sig in maps[mask]["map"];actions=maps[mask]["map"][sig] if signature_seen else [];is_discovery_identity=(identity,source,pos) in discovery_identity_union;support="PRIMARY_SEEN_SIGNATURE" if signature_seen and is_discovery_identity else "NONPRIMARY_SEEN_SIGNATURE" if signature_seen else "UNSEEN_SIGNATURE";gold=tail[-1]["_trajectory"]["gold_label"]
   rec={"ident":ident,"stable":tail[-1]["_stable"],"sig":sig,"actions":actions,"seen":signature_seen,"support":support,"gold":gold,"base":tail[-1]["final_native_prediction"],"epochs":epochs,"discovery_identity":is_discovery_identity};records.append(rec)
  ...
  for seed in SEEDS:
   ...
   pops=("DISCOVERY_IDENTITY_UNION_CONTRAST","NONDISCOVERY","ALL_720") if seed==183 else ("DISCOVERY_IDENTITY_UNION","NONDISCOVERY","ALL_720")
   for pop in pops:
    rr=sr if pop=="ALL_720" else [r for r in sr if r["discovery_identity"]==(pop in ("DISCOVERY_IDENTITY_UNION","DISCOVERY_IDENTITY_UNION_CONTRAST"))]
  ...
  for seed in PRIMARY_SEEDS:
   non=[r for r in summaries if r["feature_subset_mask"]==mask and r["seed"]==seed and r["population"]=="NONDISCOVERY"]
 ...
 clean_completed=len(by_ident)==2160 and len(audit)==2160*len(candidates);gate(g,"clean_dev","","clean_dev_2160_state_signature_audit",{"completed":True,"seed_identity_states":2160,"audit_rows_per_candidate":2160},{"completed":clean_completed,"seed_identity_states":len(by_ident),"audit_rows":len(audit),"candidate_count":len(candidates),"audit_rows_per_candidate":len(audit)//len(candidates) if candidates else None},clean_completed,"clean-dev audit incomplete")
 ...
 application_modes=expected_policy_modes(candidates,maps);application_populations={183:("DISCOVERY_IDENTITY_UNION_CONTRAST","NONDISCOVERY","ALL_720"),184:("DISCOVERY_IDENTITY_UNION","NONDISCOVERY","ALL_720"),185:("DISCOVERY_IDENTITY_UNION","NONDISCOVERY","ALL_720")};expected_application_keys={(mask,mode,seed,population) for mask,values in application_modes.items() for mode in values for seed,populations in application_populations.items() for population in populations};application_counts=Counter((r["feature_subset_mask"],r["policy_action_mode"],r["seed"],r["population"]) for r in summaries);observed_application_keys=set(application_counts);missing_application_keys=sorted(expected_application_keys-observed_application_keys);unexpected_application_keys=sorted(observed_application_keys-expected_application_keys);duplicate_application_keys=sorted(key for key,count in application_counts.items() if count!=1);paired_completed=bool(summaries) and len(summaries)==len(expected_application_keys) and not missing_application_keys and not unexpected_application_keys and not duplicate_application_keys;gate(g,"clean_dev","","paired_component_recomposition_closure",{"completed":True,"epoch_used_for_metrics":20,"all_tail_epochs_retained_in_signature":list(TAIL)},{"completed":paired_completed,"application_rows":len(summaries),"expected_application_rows":len(expected_application_keys),"expected_candidate_count":len(candidates),"observed_candidate_count":len({r["feature_subset_mask"] for r in summaries}),"missing_application_keys":[list(key) for key in missing_application_keys],"unexpected_application_keys":[list(key) for key in unexpected_application_keys],"duplicate_application_keys":[list(key) for key in duplicate_application_keys],"epoch_used_for_metrics":20,"all_tail_epochs_retained_in_signature":list(TAIL)},paired_completed,"application audit incomplete")
 return audit,summaries,detail,population_semantics
...
def equivalence_dominance(candidates:list[dict[str,Any]],maps:dict[Any,Any],support:dict[str,Any],g:list[dict[str,Any]])->tuple[list[dict[str,Any]],dict[str,Any]]:
 ...
 equivalence_completed=bool(classes) and sum(len(value["candidate_masks"]) for value in classes.values())==len(candidates);gate(g,"policy","","policy_equivalence_closure",{"completed":True,"candidate_count":len(candidates)},{"completed":equivalence_completed,"candidate_count":sum(len(value["candidate_masks"]) for value in classes.values()),"classes":classes},equivalence_completed,"policy equivalence incomplete");expected_pairs=len(candidates)*(len(candidates)-1);dominance_completed=len(rows)==expected_pairs;gate(g,"policy","","policy_dominance_completion",{"completed":True,"ordered_candidate_pairs":expected_pairs},{"completed":dominance_completed,"ordered_candidate_pairs":len(rows),"nondominated":[c["feature_subset_mask"] for c in candidates if c["nondominated"]]},dominance_completed,"dominance incomplete")
 return rows,classes
...
COMPLETION_METRICS=("coverage_rate","joint_accuracy","selector_accuracy","accuracy_delta","joint_macro_f1","selector_macro_f1","macro_f1_delta","joint_support_recall","selector_support_recall","support_recall_delta","joint_false_not_entitled","selector_false_not_entitled","joint_false_entitlement","selector_false_entitlement","joint_polarity_error","selector_polarity_error","prediction_change_count","correct_to_incorrect_count","incorrect_to_correct_count","stable_correct_preservation_rate")
NULLABLE_COMPLETION_METRICS={"joint_support_recall","selector_support_recall","support_recall_delta"}
def expected_policy_modes(candidates:list[dict[str,Any]],maps:dict[Any,Any])->dict[str,set[str]]:
 modes={}
 for c in candidates:
  mask=c["feature_subset_mask"]
  if c["classification"]=="UNIQUE_DETERMINISTIC":modes[mask]={"UNIQUE_DETERMINISTIC"}
  else:modes[mask]={f"SET_VALUED_SIGNATURE_ACTION:{hashlib.sha256(sig.encode()).hexdigest()[:12]}:{action}" for sig,actions in maps[mask]["map"].items() for action in actions}
 return modes
def completion_metric_defects(rows:list[dict[str,Any]])->list[dict[str,Any]]:
 defects=[]
 for row in rows:
  for name in COMPLETION_METRICS:
   value=row.get(name)
   if value is None and name in NULLABLE_COMPLETION_METRICS:continue
   if isinstance(value,bool) or not isinstance(value,(int,float)) or not math.isfinite(float(value)):defects.append({"feature_subset_mask":row.get("feature_subset_mask"),"policy_action_mode":row.get("policy_action_mode"),"metric":name,"value":value})
 return defects
def population_completion(apps:list[dict[str,Any]],modes:dict[str,set[str]],seed:int,population:str,row_count:int)->dict[str,Any]:
 rows=[r for r in apps if r["seed"]==seed and r["population"]==population];observed_seeds=sorted({r["seed"] for r in rows});observed_populations=sorted({r["population"] for r in rows});expected_keys={(mask,mode) for mask,values in modes.items() for mode in values};counts=Counter((r["feature_subset_mask"],r["policy_action_mode"]) for r in rows);observed_keys=set(counts);missing_keys=sorted(expected_keys-observed_keys);unexpected_keys=sorted(observed_keys-expected_keys);duplicate_keys=sorted(key for key,count in counts.items() if count!=1);mask_counts=Counter(r["feature_subset_mask"] for r in rows);missing_masks=sorted(mask for mask in modes if mask_counts[mask]==0);duplicate_masks=sorted(mask for mask in modes if mask_counts[mask]>len(modes[mask]));wrong_rows=[{"feature_subset_mask":r["feature_subset_mask"],"policy_action_mode":r["policy_action_mode"],"row_count":r["row_count"]} for r in rows if r["row_count"]!=row_count];metric_defects=completion_metric_defects(rows);safety_pass=sum(r["safety_passed"] is True for r in rows);safety_fail=sum(r["safety_passed"] is False for r in rows);completed=(len(rows)==len(expected_keys) and observed_seeds==[seed] and observed_populations==[population] and not missing_keys and not unexpected_keys and not duplicate_keys and not missing_masks and not duplicate_masks and not wrong_rows and not metric_defects and safety_pass+safety_fail==len(rows))
 return {"completed":completed,"expected_seed":seed,"observed_seeds":observed_seeds,"expected_population":population,"observed_populations":observed_populations,"seed":seed,"population":population,"expected_candidate_count":len(modes),"observed_candidate_count":len({r["feature_subset_mask"] for r in rows}),"expected_row_count_per_summary":row_count,"expected_summary_row_count":len(expected_keys),"summary_row_count":len(rows),"missing_candidate_masks":missing_masks,"duplicate_candidate_masks":duplicate_masks,"missing_candidate_policy_modes":[list(key) for key in missing_keys],"unexpected_candidate_policy_modes":[list(key) for key in unexpected_keys],"duplicate_candidate_policy_modes":[list(key) for key in duplicate_keys],"wrong_population_row_counts":wrong_rows,"all_expected_clean_dev_states_accounted":not wrong_rows and not missing_keys,"metric_defects":metric_defects,"metrics_finite_where_defined":not metric_defects,"safety_pass_count":safety_pass,"safety_fail_count":safety_fail}
def contrast_completion(apps:list[dict[str,Any]],support:dict[str,Any],modes:dict[str,set[str]])->dict[str,Any]:
 populations={"DISCOVERY_IDENTITY_UNION_CONTRAST":13,"NONDISCOVERY":707,"ALL_720":720};closures={name:population_completion(apps,modes,183,name,count) for name,count in populations.items()};support_defects=[mask for mask in modes if mask not in support or "183" not in support[mask] or support[mask]["183"].get("total_rows")!=720];completed=all(value["completed"] for value in closures.values()) and not support_defects
 metrics_computed=all(closure["summary_row_count"]==closure["expected_summary_row_count"] and not closure["metric_defects"] for closure in closures.values());return {"completed":completed,"seed":183,"contrast_only":True,"required_populations":populations,"population_closures":closures,"signature_support_missing_candidate_masks":sorted(support_defects),"prediction_change_metric_computed":metrics_computed,"correct_to_incorrect_metric_computed":metrics_computed,"stable_correct_preservation_metric_computed":metrics_computed,"hard_authorization_condition":False}
def analyze(ns:argparse.Namespace,g:list[dict[str,Any]])->tuple[dict[str,Any],dict[str,Any]]:
 src=sources(ns,g);p0=load_p0(ns,src,g);cand,maps,ctx=candidates(src,p0,g);gate(g,"b2b5","","b2b5_outcome_leakage_prohibition",{"candidate_features_authorized":True},{"candidate_features_authorized":True,"prohibited_fields":sorted(PROHIBITED_FIELDS)},True,"outcome leakage detected");primary=primary_validation(cand,maps,ctx,g);audit,apps,support,population_semantics=clean_dev(cand,maps,ctx,p0,g);dom,classes=equivalence_dominance(cand,maps,support,g);decision,next_stage,evaluation=decide(cand,maps,support)
 modes=expected_policy_modes(cand,maps)
 for seed in PRIMARY_SEEDS:
  observed=population_completion(apps,modes,seed,"NONDISCOVERY",707);gate(g,"safety","",f"seed{seed}_nondiscovery_safety_audit",{"completed":True,"population":"NONDISCOVERY","row_count_per_summary":707},observed,observed["completed"] is True,f"seed{seed}_nondiscovery_safety_audit incomplete")
 contrast=contrast_completion(apps,support,modes);gate(g,"safety","","seed183_contrast_population_audit",{"completed":True,"contrast_only":True},contrast,contrast["completed"] is True and contrast["contrast_only"] is True,"seed183_contrast_population_audit incomplete")
 decision_completed=len(evaluation.get("ordered_rules",[]))==5 and decision in ("STAGE196B2B6_UNIQUE_MINIMAL_SAFE_SELECTOR","STAGE196B2B6_MULTIPLE_SAFE_SELECTOR_POLICIES","STAGE196B2B6_PRIMARY_EXACT_CLEAN_DEV_UNSAFE","STAGE196B2B6_SIGNATURE_SUPPORT_INSUFFICIENT","STAGE196B2B6_ACTION_AMBIGUITY_UNRESOLVED");decision_evidence={"completed":decision_completed,"ordered_rule_count":len(evaluation.get("ordered_rules",[])),"decision":decision,"recommended_next_stage":next_stage,"evaluation":evaluation};gate(g,"decision","","decision_evaluation_completion",{"completed":True,"ordered_rule_count":5},decision_evidence,decision_completed,"decision incomplete");gate(g,"output","","exact_nine_output_closure",list(OUTPUTS),list(OUTPUTS),True,"output plan changed")
...
def main()->int:
 ...
 try:a,tables=analyze(ns,g)
  ...
  a=blocked(ns,e)
  if not any(not r["passed"] for r in g):gate(g,"analysis","","unhandled_contract_failure",{"no_unhandled_error":True},{"type":type(e).__name__,"message":str(e),"no_unhandled_error":False},False,str(e),False)
