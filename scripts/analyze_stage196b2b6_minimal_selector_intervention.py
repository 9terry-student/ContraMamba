...
def clean_dev(candidates:list[dict[str,Any]],maps:dict[Any,Any],ctx:dict[str,Any],p0:dict[str,Any],g:list[dict[str,Any]])->tuple[list[dict[str,Any]],list[dict[str,Any]],dict[str,Any]]:
 ...
 for c in candidates:
  ...
  for seed in PRIMARY_SEEDS:
   non=[r for r in summaries if r["feature_subset_mask"]==mask and r["seed"]==seed and r["population"]=="NONPRIMARY_704"]
...
def contrast_completion(apps:list[dict[str,Any]],support:dict[str,Any],modes:dict[str,set[str]])->dict[str,Any]:
 populations={"CONTRAST_PRIMARY_IDENTITIES":16,"NONPRIMARY":704,"ALL_720":720};closures={name:population_completion(apps,modes,183,name,count) for name,count in populations.items()};support_defects=[mask for mask in modes if mask not in support or "183" not in support[mask] or support[mask]["183"].get("total_rows")!=720];completed=all(value["completed"] for value in closures.values()) and not support_defects
...
def analyze(ns:argparse.Namespace,g:list[dict[str,Any]])->tuple[dict[str,Any],dict[str,Any]]:
 ...
 for seed in PRIMARY_SEEDS:
  observed=population_completion(apps,modes,seed,"NONPRIMARY_704",704);gate(g,"safety","",f"seed{seed}_nonprimary_safety_audit",{"completed":True},observed,observed["completed"] is True,f"seed{seed}_nonprimary_safety_audit incomplete")
 contrast=contrast_completion(apps,support,modes);gate(g,"safety","","seed183_contrast_audit",{"completed":True,"contrast_only":True},contrast,contrast["completed"] is True and contrast["contrast_only"] is True,"seed183_contrast_audit incomplete")
