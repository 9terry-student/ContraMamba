...
def clean_dev(candidates:list[dict[str,Any]],maps:dict[Any,Any],ctx:dict[str,Any],p0:dict[str,Any],g:list[dict[str,Any]])->tuple[list[dict[str,Any]],list[dict[str,Any]],dict[str,Any]]:
 ...
 audit=[];summaries=[];detail={}
 for c in candidates:
  mask=c["feature_subset_mask"];members=maps[mask]["members"];records=[]
  for ident,epochs in sorted(by_ident.items()):
   seed,identity,source,pos=ident;tail=[epochs[e]["joint"] for e in TAIL];sig=canon(signature(members,tail));actions=maps[mask]["map"].get(sig,[]);is_primary=(identity,source,pos) in primary_identities;support="PRIMARY_SEEN_SIGNATURE" if actions and is_primary else "NONPRIMARY_SEEN_SIGNATURE" if actions else "UNSEEN_SIGNATURE";gold=tail[-1]["_trajectory"]["gold_label"]
   rec={"ident":ident,"stable":tail[-1]["_stable"],"sig":sig,"actions":actions,"support":support,"gold":gold,"base":tail[-1]["final_native_prediction"],"epochs":epochs,"primary":is_primary};records.append(rec)
   audit.append({"feature_subset_mask":mask,"seed":seed,"stable_row_id":rec["stable"],"id":identity,"source_row_id":source,"dev_position":pos,"gold_label":gold,"recipient_tail_predictions":[r["final_native_prediction"] for r in tail],"signature":json.loads(sig),"support_class":support,"assigned_action_set":actions if actions else ["00000"],"abstained":not actions})
  detail[mask]={}
  for seed in SEEDS:
   sr=[r for r in records if r["ident"][0]==seed];seen=sum(bool(r["actions"]) for r in sr);detail[mask][str(seed)]={"total_rows":720,"seen_rows":seen,"unseen_rows":720-seen,"coverage_rate":seen/720,"signature_count":len({r["sig"] for r in sr}),"unseen_signature_count":len({r["sig"] for r in sr if not r["actions"]})}
   ...
   for pop in pops:
    ...
    for mode,sig_filter,action_filter in interpretations:
     ...
     for r in rr:
      action=(r["actions"][0] if mode=="UNIQUE_DETERMINISTIC" and r["actions"] else action_filter if r["sig"]==sig_filter else None)
      sel.append(apply_mask(r["epochs"][20]["joint"],r["epochs"][20]["frame_local_only"],action)["prediction"] if action else r["base"])
     m=metrics(gold,base,sel);seen_count=sum(bool(r["actions"]) for r in rr);summaries.append({"feature_subset_mask":mask,"seed":seed,"population":pop,"policy_action_mode":mode,"row_count":len(rr),"seen_signature_count":seen_count,"unseen_signature_count":len(rr)-seen_count,"coverage_rate":seen_count/len(rr) if rr else None,**m,"safety_passed":m["correct_to_incorrect_count"]==0 and m["stable_correct_preservation_rate"]==1.0})
 ...
 gate(g,"clean_dev","","clean_dev_2160_state_signature_audit",2160,{"states":len(by_ident),"audit_rows":len(audit),"candidates":len(candidates)},len(audit)==2160*len(candidates),"clean-dev audit incomplete");gate(g,"clean_dev","","unseen_signature_abstention_closure","00000",{"violations":sum(r["abstained"] is False and not r["assigned_action_set"] for r in audit)},all(r["abstained"]== (r["assigned_action_set"]==["00000"]) for r in audit),"abstention closure failed");gate(g,"clean_dev","","paired_component_recomposition_closure",True,{"application_rows":len(summaries),"epoch_used_for_metrics":20,"all_tail_epochs_retained_in_signature":list(TAIL)},bool(summaries),"application audit incomplete")
