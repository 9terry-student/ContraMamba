...
DESIGN_ARTIFACT = Path(
    "reports/reason_router_gen4_precursor_v3_numerically_stable_dcs_design_candidate.md"
)
DESIGN_GIT_BLOB = "e313ba71e8c796e73786209a340b62737b9bbb91"
...
# Precursor-v3 changes exactly one measurement constant. The imported
# Precursor-v2 DCS module remains frozen at 0.025 outside the scoped context.
V3_EPSILON = 0.05
...
REPORT_FILE = "precursor_v3_dcs_one_row_equivalence_report.json"
SUMS_FILE = "SHA256SUMS.txt"
REPORT_SCHEMA = "gen4-precursor-v3-dcs-one-row-equivalence-v1"
RESULT_PASS = "PASS_PRECURSOR_V3_DCS_ONE_ROW_CPU_CUDA_EQUIVALENCE"


class PrecursorV3DCSEquivalenceError(RuntimeError):
    pass


def require(ok: bool, message: str) -> None:
    if not ok:
        raise PrecursorV3DCSEquivalenceError(message)
...
def git(*args: str) -> str:
    try:
        ...
        raise PrecursorV3DCSEquivalenceError(
...
@contextmanager
def v3_epsilon_context():
    original = float(dcs.EPSILON)
    require(original == 0.025, f"V2_SUBSTRATE_EPSILON:{original}")
    dcs.EPSILON = V3_EPSILON
    try:
        yield
    finally:
        dcs.EPSILON = original
...
def validate_protocol() -> None:
    ...
    require(dcs.EPSILON == 0.025, "V2_SUBSTRATE_EPSILON")
    require(V3_EPSILON == 0.05, "V3_EPSILON")
...
def historical_equivalence_input(
    *,
    ...
    provenance = {
        ...
        "new_precursor_v3_scientific_cohort_accessed": False,
    }
...
def run_probe_set(
    *,
    ...
    require(dcs.EPSILON == 0.025, "V2_EPSILON_NOT_RESTORED")
    ...
    require(
        all(float(p["epsilon"]) == V3_EPSILON for p in probes),
        "V3_PROBE_EPSILON",
    )
    return probes
...
def compare_probe_sets(
    ...
    for key in sorted(cpu_by_key):
        ...
        gpu = gpu_by_key[key]
        require(
            float(cpu["epsilon"]) == float(gpu["epsilon"]) == V3_EPSILON,
            f"EPSILON:{key}",
        )
...
def run_gate(
    *,
    ...
    report = {
        ...
        "redesign": {
            "precursor_v2_scientific_execution_observed": False,
            "reason": "backend_equivalence_numerical_conditioning_failure",
            "only_changed_measurement_constant": "epsilon",
            "previous_epsilon": 0.025,
            "v3_epsilon": V3_EPSILON,
...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "for the prospective Precursor-v3 dynamic causal susceptibility "
            "measurement. Uses the first decisive-gold row from the old "
            "historical precursor population and only offset t*-4. "
            "Does not access the frozen N=800 scientific cohort."
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("NEW_PRECURSOR_V3_SCIENTIFIC_COHORT_ACCESSED=False")
