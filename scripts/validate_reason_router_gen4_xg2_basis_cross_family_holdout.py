...
def authenticate_authority() -> None:
    ...
    require(
        rc == 0,
        "SCOPE_FREEZE_NOT_ANCESTOR",
    )

    frozen_scope_blob = git(
        "rev-parse",
        f"{SCOPE_FREEZE_COMMIT}:{SCOPE_REL.as_posix()}",
    )

    require(
        frozen_scope_blob == SCOPE_BLOB,
        f"SCOPE_FROZEN_BLOB:{frozen_scope_blob}",
    )
    ...
    require(
        head_scope_blob == SCOPE_BLOB,
        f"SCOPE_HEAD_DRIFT:{head_scope_blob}",
    )
    ...
    # HEAD blob identity is pinned above. Use Git's own diff semantics
    # for worktree/index drift so checkout newline conversion (e.g. CRLF
    # on Windows) does not create a false provenance failure.
    for diff_args, state in (
        ...
        require(
            rc == 0,
            f"CALIBRATION_BUILDER_{state}_DRIFT",
        )
...
def verify_calibration_regeneration() -> None:
    for key in holdouts.FAMILY_KEYS:
        ...
        # Git checkout filters may materialize tracked JSONL with CRLF
        # on Windows. Compare parsed content here and retain the frozen
        # canonical regeneration SHA256 checks below.
        require(
            read_jsonl(source_path) == facts,
            f"{key}:CALIBRATION_SOURCE_CONTENT_DRIFT",
        )
        require(
            read_jsonl(row_path) == rows,
            f"{key}:CALIBRATION_ROW_CONTENT_DRIFT",
        )
...
def validate_and_write(
    ...
    for key in holdouts.FAMILY_KEYS:
        ...
        manifest = {
            ...
            "absolute_schedule_indexing_601_900": True,
...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "XG2-basis cross-family 601..900 holdout. "
