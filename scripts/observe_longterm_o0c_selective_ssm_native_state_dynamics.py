...
def measurements(rows:Sequence[Mapping[str,Any]],vectors:Sequence[Any],anchors:Mapping[tuple[str,str],Mapping[str,int]])->list[dict[str,Any]]:
    matrix=np.ascontiguousarray(np.asarray(vectors,dtype="<f4")); validate_state_rows(rows,matrix); lookup={(r["pair_id"],r["condition"],r["layer_index"],r["absolute_token_index"]):r for r in rows}; out=[]
    for pair in PAIR_ORDER:
     for comparison,member in COMPARISONS:
      for layer in sorted({r["layer_index"] for r in rows}):
       dminus=anchors[(pair,member)].get("anchor_pre_minus_1"); rdminus=anchors[(pair,"reference_sufficient")].get("anchor_pre_minus_1")
       require(type(dminus) is int and dminus==rdminus,"pre-divergence anchor")
