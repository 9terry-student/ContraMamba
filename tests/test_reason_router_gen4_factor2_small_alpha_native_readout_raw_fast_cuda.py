...
lowdisp_stub = types.ModuleType(
    "scripts.reason_router_gen4_factor2_small_alpha_runtime_inputs"
)
...
def test_protocol_constants_are_exact() -> None:
    assert runner.REQUIRED_BASE_COMMIT == (
        "9b0dfe91780de00347ce3827102f08210d6b558e"
    )
...
def test_output_contract_is_four_files() -> None:
    assert runner.ITEM_FILE == "factor2_small_alpha_native_readout_items.jsonl"
...
def test_runner_uses_fresh_response_blind_runtime_helper() -> None:
    ...
    assert (
        "reason_router_gen4_mamba370m14b_"
        "low_displacement_behavioral_raw_fast_cuda"
        not in source
    )
    assert "xg1_fact_5701..xg1_fact_6000" in source
    assert (
