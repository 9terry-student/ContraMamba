...
def test_protocol_is_exact() -> None:
    ...
    assert subject.SHARDS[0]["pair_first"] == "xg1_fact_7501"
    assert subject.SHARDS[0]["pair_last"] == "xg1_fact_7650"
    assert subject.SHARDS[1]["pair_first"] == "xg1_fact_7651"
    assert subject.SHARDS[1]["pair_last"] == "xg1_fact_7800"
...
def test_frozen_readout_population_is_exact() -> None:
    ...
    assert facts[0]["pair_id"] == "xg1_fact_7501"
    assert facts[-1]["pair_id"] == "xg1_fact_7800"
...
def test_model_checkpoint_and_layer_identity() -> None:
    ...
    assert subject.geom.SOURCE_BLOCK == 33
    assert subject.geom.TARGET_RESIDUAL_LAYER == 34
    assert subject.geom.INTERVENTION_LAYER == 35
    assert subject.geom.TARGET_OFFSET == 2
    assert subject.geom.INTERMEDIATE_SIZE == 3072
...
def test_plane_components_use_p2_coefficients_for_p5_control(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        subject,
