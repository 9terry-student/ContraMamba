...
from scripts import (
    reason_router_gen4_pp3_excluded_residual_individual_plane_restoration_sufficiency_localization_fast_cuda as r
)
...
def synthetic_audit(condition: str) -> dict:
    ...
    if plane is None:
        ...
        a, b = coeff[plane]
        component_l2 = (a * a + b * b) ** 0.5
        quarter_l2 = component_l2
        mismatch = 0.0
        dot = 0.0
        correction_l2 = (
            component_l2
            if kind == "neutralized"
            else (2.0 ** 0.5) * component_l2
        )
...
SCALES = {
    "native": 1.0,
    "p1_neutralized": .80,
    "p1_quarter_turn_replacement": .95,
    "p2_neutralized": .82,
    "p2_quarter_turn_replacement": .94,
    "p4_neutralized": .84,
    "p4_quarter_turn_replacement": .93,
    "p5_neutralized": .86,
    "p5_quarter_turn_replacement": .92,
}
...
def item(index: int) -> dict:
    ...
    return {
        ...
        **r.endpoints_from_conditions(by),
...
def summary():
    ...
    shards = [
        {
            "shard_id": s["shard_id"],
            "gpu_id": s["gpu_id"],
            "device_name": f"synthetic-gpu-{s['gpu_id']}",
            "pair_first": s["pair_first"],
            "pair_last": s["pair_last"],
    ...
    return {
        ...
        "pair_id_first": "xg1_fact_2101",
        "pair_id_last": "xg1_fact_2400",
        "epsilon": r.EPS,
        "residual_plane_order": list(r.RESIDUAL_PLANES),
        "condition_order": list(r.CONDITIONS),
        "direction_order": list(r.DIRECTIONS),
        ...
        "planned_multiplicity_method":
            r.PLANNED_MULTIPLICITY,
        "planned_familywise_alpha":
            r.PLANNED_FWER_ALPHA,
        "primary_inference_executed": False,
        "multiplicity_correction_executed": False,
...
def rewrite_artifact_hashes(out: Path, changed_name: str, raw: bytes):
    ...
    manifest["files"][changed_name]["sha256"] = (
...
def test_frozen_constants_shards_and_budget():
    assert r.DESIGN_COMMIT == (
        "8c0ff6dbad77ed876fc1481b3b53c3fd47a27d3b"
    )
    assert r.STATIC_COMMIT == (
        "4a1d5871fad17a34951bc433a283c95e682a081b"
    )
    assert r.AUTHORITY_COMMIT == (
        "25ff56c80ccd943bb386ebc7ab6612fe6e68d470"
    )
    assert r.AUTHORITY_BLOB == (
        "894d665dc89f41435363f55d0f4eadc3a3b24402"
    )
    ...
    assert r.expected_pairs()[0] == "xg1_fact_2101"
    assert r.expected_pairs()[-1] == "xg1_fact_2400"
    assert r.F_PAIR == 360
    assert r.F_TOTAL == 108000
    assert r.SHARDS[0]["pair_first"] == "xg1_fact_2101"
    assert r.SHARDS[0]["pair_last"] == "xg1_fact_2250"
    assert r.SHARDS[1]["pair_first"] == "xg1_fact_2251"
    assert r.SHARDS[1]["pair_last"] == "xg1_fact_2400"
...
def test_frozen_static_inputs_and_plane_authentication():
    ...
    assert (
        validated["preparation"]["scientific_model_forward_count"]
        == 0
    )
    assert validated["preparation"]["checkpoint_load_count"] == 0
    assert validated["preparation"]["gpu_used"] is False
    assert (
        validated["preparation"]["future_execution_contract"]
...
def test_neutralization_and_quarter_turn_replacement_math(plane):
    ...
    neutral = r.condition_correction(
        h,
        r.neutral_condition(plane),
        planes,
    )
    replacement = r.condition_correction(
        h,
        r.replacement_condition(plane),
        planes,
    )
    ...
    expected_replacement = torch.zeros(
        r.DIM,
        dtype=torch.float64,
    )
    # -c_k + r_k
    expected_replacement[offset] = -a - b
    expected_replacement[offset + 1] = -b + a
    ...
    post = h + replacement["d"]
    assert float(post[offset]) == -b
    assert float(post[offset + 1]) == a
    ...
    assert abs(neutral["native_quarter_turn_dot"]) <= r.TOL
    assert abs(replacement["native_quarter_turn_dot"]) <= r.TOL

    assert neutral["pp3_coefficient_drift_max_abs"] <= r.TOL
    assert replacement["pp3_coefficient_drift_max_abs"] <= r.TOL

    assert neutral["other_residual_plane_drift_max_abs"] <= r.TOL
    assert replacement["other_residual_plane_drift_max_abs"] <= r.TOL
...
def test_native_condition_is_exact_restoration_state():
    ...
    assert value["native_component_l2"] is None
    assert value["quarter_turn_component_l2"] is None


def test_restoration_endpoint_identity():
    ...
    assert result["S"] == 10.0 - result["QB"]
    assert result["S_C"] == result["QC"] - result["QB"]
    assert result["D_SUF"] == 10.0 - result["QC"]


def test_d_suf_uses_canonical_q0_minus_qc_operation():
    q0 = 0.1
    qb = 0.2
    qc = 100000000.0

    expanded = (q0 - qb) - (qc - qb)
    canonical = q0 - qc

    assert expanded != canonical

    result = r.plane_endpoint(q0, qb, qc)
    assert result["S"] == q0 - qb
    assert result["S_C"] == qc - qb
    assert result["D_SUF"] == canonical
...
def test_artifact_roundtrip_and_endpoint_mutation_rejected(
    tmp_path: Path,
):
    ...
    rows[0]["plane_endpoints"]["P1"]["D_SUF"] += 1.0

    raw = r.jsonl(rows)
    rewrite_artifact_hashes(
        out,
        r.ITEM_FILE,
        raw,
    )
    ...
    with pytest.raises(
        r.ResidualIndividualPlaneRestorationSufficiencyError,
        match="ENDPOINT:P1:D_SUF",
    ):
        r.validate_artifact(out)


def test_raw_artifact_rejects_provenance_order_and_budget_mutations(
    tmp_path: Path,
):
    items = [item(i) for i in range(r.N)]
    ...
    value = json.loads(
        (out3 / r.SUMMARY_FILE).read_text(encoding="utf-8")
    )
...
def test_runner_has_no_statistical_inference_and_is_device_generic():
    ...
    assert (
        "make_fast_capture_for_device(runtime,kernels,device)"
        in compact
    )
    ...
    # Critical semantic regression: matched replacement is -c_k + r_k.
    assert 'delta_c=(-component+quarter).contiguous()' in compact

    # Critical endpoint regression: raw D_SUF is canonical Q0-QC.
    assert '"D_SUF":q0-qc' in compact
