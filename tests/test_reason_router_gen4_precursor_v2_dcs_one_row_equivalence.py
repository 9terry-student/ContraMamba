...
from scripts import (
    reason_router_gen4_precursor_v2_dcs_one_row_equivalence
    as eq,
)
...
def test_compare_scalar_uses_frozen_atol_rtol() -> None:
    ...
    with pytest.raises(eq.PrecursorV2DCSEquivalenceError):
...
def test_source_never_reads_new_precursor_v2_cohort() -> None:
    ...
    assert '"new_precursor_v2_cohort_accessed": False' in source
