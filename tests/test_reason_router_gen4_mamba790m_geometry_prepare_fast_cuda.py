...
def test_pinned_cross_backbone_identity_constants():
    ...
    assert subject.HF_REPO == "state-spaces/mamba-790m-hf"
    assert (
        subject.HF_REVISION
    ...
    # The 790M tokenizer bytes were independently authenticated during the
    # constructor preflight and must remain the exact active-encoding bytes.
    assert subject.HIDDEN_SIZE == 1536
    assert subject.INTERMEDIATE_SIZE == 3072
...
def test_geometry_stage_has_no_xg1_response_gate():
    assert subject.RESULT_PASS == "PASS_MAMBA790M_GEOMETRY_PREPARATION"
