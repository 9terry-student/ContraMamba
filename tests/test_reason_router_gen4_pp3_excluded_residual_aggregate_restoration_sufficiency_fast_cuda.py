...
def synthetic_audit(condition: str) -> dict:
    ...
    component_l2 = (
        sum(x * x for pair in coefficients.values() for x in pair)
        ** 0.5
    )
    projections = {
        plane: [0.0, 0.0]
    ...
    return {
        ...
        "quarter_turn_component_l2": component_l2,
        "restoration_replacement_addition_l2_mismatch": 0.0,
        "native_quarter_turn_dot": 0.0,
        "condition_correction_l2":
            0.0 if condition == "native" else component_l2,
...
SCALES = {
    "native": 1.0,
    "residual_neutralized": .8,
    "residual_quarter_turn_replacement": .95,
}
...
def item(index: int) -> dict:
    ...
    endpoint = r.endpoint(
        ...
        float(by["residual_quarter_turn_replacement"]["Q"]),
    )
...
def summary():
    ...
    return {
        ...
        "pair_id_first": "xg1_fact_2401",
        "pair_id_last": "xg1_fact_2700",
        "epsilon": r.EPS,
        "residual_plane_order": list(r.RESIDUAL_PLANES),
        "condition_order": list(r.CONDITIONS),
        "direction_order": list(r.DIRECTIONS),
...
def test_frozen_constants_shards_and_budget():
    assert r.DESIGN_COMMIT == "14d71b742488dd088ca22c416962522235d8d67b"
    assert r.STATIC_COMMIT == "7744ddabe05e0180428d3e753a4453dea531ef6a"
    assert r.AUTHORITY_COMMIT == "ca4a0cd25e6fe21b79ac8c545d68ec60e26fefcd"
    assert r.AUTHORITY_BLOB == "aacf3d68804d85640372f1522138ca4f84104c6b"
    ...
    assert r.expected_pairs()[0] == "xg1_fact_2401"
    assert r.expected_pairs()[-1] == "xg1_fact_2700"
    assert r.F_PAIR == 120
    assert r.F_TOTAL == 36000
    assert r.SHARDS[0]["pair_first"] == "xg1_fact_2401"
    assert r.SHARDS[0]["pair_last"] == "xg1_fact_2550"
    assert r.SHARDS[1]["pair_first"] == "xg1_fact_2551"
    assert r.SHARDS[1]["pair_last"] == "xg1_fact_2700"
...
def test_aggregate_neutralization_and_restoration_replacement_math():
    ...
    component = torch.zeros(r.DIM, dtype=torch.float64)
    component[:8] = h[:8]

    quarter = torch.zeros(r.DIM, dtype=torch.float64)
    ...
    assert torch.equal(neutral["d"], -component)
    assert torch.equal(
        replacement["d"],
        -component + quarter,
    )
    ...
    assert (
        neutral["residual_neutralization_max_abs_projection"]
        == 0.0
    )
    assert (
        replacement["replacement_coordinate_max_abs_residual"]
        == 0.0
    )
    assert (
        replacement[
...
def test_restoration_endpoint_identity_and_canonical_operation():
    ...
    result = r.endpoint(q0, qb, qc)
    assert result["D_RES_SUF"] == canonical
    assert result["S_R"] == q0 - qb
    assert result["S_C"] == qc - qb

    # This deliberately extreme scale demonstrates why the canonical
    # endpoint must be stored as Q0-Q_C rather than via cancellation.
    # The normal-scale case above already exercises validate_endpoint().


def test_no_baseline_forward_accounting_and_item_validation():
    value = item(0)
...
def test_artifact_roundtrip_and_endpoint_mutation_rejected(
    ...
    with pytest.raises(
        r.ResidualAggregateRestorationSufficiencyError,
        match="ENDPOINT:D_RES_SUF",
    ):
...
def test_raw_artifact_rejects_provenance_order_and_budget_mutations(
    ...
    value["condition_order"] = list(
        reversed(r.CONDITIONS)
    )
    raw = r.canonical(value)
    rewrite_artifact_hashes(
        out2, r.SUMMARY_FILE, raw
    )
    with pytest.raises(
        r.ResidualAggregateRestorationSufficiencyError,
        match="SUMMARY_ORDER",
...
def test_runner_has_no_statistical_inference_and_is_device_generic():
    ...
    assert '"quarter_turn_control"' not in source

    assert '"D_RES_SUF":q0-qc' in compact
    assert '"D_RES_NEC"' not in source
